...
// Legacy ROMs (<= v32.3) encode the xflag slice in the overlay type (OV_XFLAG0 + sliceId), so the
// full range is still used when expanding a resolved xflag identity. Newer ROMs only ever send the
// base OV_XFLAG (0x10) with a compact XflagID; ResolveXflagItem re-derives the slice (see Combo/Xflags).
...
/* Base xflag overlay type, as sent by ROMs using the compact-XflagID system (== OV_XFLAG0). */
#define OV_XFLAG        OV_XFLAG0

// Informational only (not read by the tracker). Bit positions track current OoTMM (OVF_RENEW = bit 0,
// OVF_PRECOND = bit 1); older builds used different bits.
