...
typedef struct SceneMetaInfo
{
	...
	/* Optional alternate map shown when the context toggle is ON (Spring / Adult).
	   NULL / empty => ImagePath is used for both contexts (current behaviour), so
	   context-less scenes and scenes without a per-context map are unaffected. */
	const char* ContextImagePath;
...
#define MM_SPRING_WATER_CAVE					MM_GORON_VILLAGE + 1

#define MM_NUM_SCENES							MM_SPRING_WATER_CAVE + 1
