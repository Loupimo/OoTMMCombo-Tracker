...
enum ObjectType
{
    ...
    silverboulder = 30,
    boulder = 31,
	gossip = boulder + 1,			// Gossip stone fairy (small)
	gossip_big = gossip + 1,		// Gossip stone great fairy (big)
...
const QSet<ObjectType> OoTTypes =
{
	...
	ObjectType::rock, ObjectType::soil, ObjectType::tree, ObjectType::bush, ObjectType::gossip, ObjectType::gossip_big,
...
const QSet<ObjectType> MMTypes =
{
	...
	ObjectType::barrel, ObjectType::heart, ObjectType::fairy, ObjectType::fairy_spot, ObjectType::icicle, ObjectType::boulder, ObjectType::redboulder, ObjectType::rock, ObjectType::soil, ObjectType::tree, ObjectType::bush, ObjectType::gossip, ObjectType::gossip_big,
...
/*
*   Legacy-scene remap entry: for a check whose true (current) scene differs from the scene that
*   pre-migration ROMs (<= v32.3) report, this records the old scene it is reported under and the
*   true scene its ObjectInfo now lives in, keyed by the object's unchanged legacy id.
*/
typedef struct LegacySceneRemap
{
	uint32_t LegacyScene;	// The scene an old ROM reports for this check.
	uint32_t ObjectID;		// The check's legacy ObjectID (unchanged by the remap).
	ObjectType Type;		// The check's object type.
	uint32_t TrueScene;		// The scene the ObjectInfo actually lives in now.
} LegacySceneRemap;

/* The per-game legacy-scene remap tables, generated into OoT/MMObjectScene.cpp by gen_objects.py. */
extern LegacySceneRemap* OoTLegacySceneRemap;
extern const size_t OoTLegacySceneRemapCount;
extern LegacySceneRemap* MMLegacySceneRemap;
extern const size_t MMLegacySceneRemapCount;
...
/*
*   Find the world-0 template object for a compact XflagID (new xflag ROMs > v32.3 / dev). The
*   lookup table is built lazily from the static scene arrays on first use.
*
*   @param XflagID	The compact XflagID to resolve.
*
*	@return The matching template object, or nullptr if no object carries that XflagID.
*/
