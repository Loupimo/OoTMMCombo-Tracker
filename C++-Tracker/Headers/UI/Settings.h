...
public:
    ...
    /* True when the spoiler was produced by a stable build <= v32.3, i.e. before the xflag rework
       that replaced the full scene/room/actor identity in the override key with a compact XflagID.
       Legacy ROMs need no XflagID resolution; newer ones (and dev / unknown builds) do. Published to
       the item pipeline via SetUsesLegacyXflags (see Combo/Xflags). */
    bool UsesLegacyXflags = false;
	ROMGame Game;
	GameMode Mode;
    GoalMode Goal;
	size_t NumOfTeams;
