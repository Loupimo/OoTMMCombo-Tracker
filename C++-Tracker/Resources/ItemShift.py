import csv
import pandas as pd 


input_file = "ID-Names.csv"   # Ton fichier source
output_file = "output.csv" # Le fichier résultat

itemOrder = [
"OOT_BOMBS_5",
"OOT_NUTS_5",
"OOT_BOMBCHU_10",
"OOT_BOW",
"OOT_SLINGSHOT",
"OOT_BOOMERANG",
"OOT_STICK",
"OOT_HOOKSHOT",
"OOT_LONGSHOT",
"OOT_LENS",
"OOT_ZELDA_LETTER",
"OOT_OCARINA_TIME",
"OOT_HAMMER",
"OOT_COJIRO",
"OOT_BOTTLE_EMPTY",
"OOT_POTION_RED",
"OOT_POTION_GREEN",
"OOT_POTION_BLUE",
"OOT_FAIRY",
"OOT_BOTTLE_MILK",
"OOT_BOTTLE_RUTO_LETTER",
"OOT_MAGIC_BEAN",
"OOT_MASK_SKULL",
"OOT_MASK_SPOOKY",
"OOT_CHICKEN",
"OOT_MASK_KEATON",
"OOT_MASK_BUNNY",
"OOT_MASK_TRUTH",
"OOT_POCKET_EGG",
"OOT_POCKET_CUCCO",
"OOT_ODD_MUSHROOM",
"OOT_ODD_POTION",
"OOT_POACHER_SAW",
"OOT_BROKEN_GORON_SWORD",
"OOT_PRESCRIPTION",
"OOT_EYEBALL_FROG",
"OOT_EYE_DROPS",
"OOT_CLAIM_CHECK",
"OOT_SWORD_KOKIRI",
"OOT_SWORD_KNIFE",
"OOT_SHIELD_DEKU",
"OOT_SHIELD_HYLIAN",
"OOT_SHIELD_MIRROR",
"OOT_TUNIC_GORON",
"OOT_TUNIC_ZORA",
"OOT_BOOTS_IRON",
"OOT_BOOTS_HOVER",
"OOT_QUIVER2",
"OOT_QUIVER3",
"OOT_BOMB_BAG",
"OOT_BOMB_BAG2",
"OOT_BOMB_BAG3",
"OOT_SILVER_GAUNTLETS",
"OOT_GOLDEN_GAUNTLETS",
"OOT_SCALE_SILVER",
"OOT_SCALE_GOLDEN",
"OOT_STONE_OF_AGONY",
"OOT_GERUDO_CARD",
"OOT_OCARINA_FAIRY",
"OOT_DEKU_SEEDS_5",
"OOT_HEART_CONTAINER",
"OOT_HEART_PIECE",
"OOT_BOSS_KEY",
"OOT_COMPASS",
"OOT_MAP",
"OOT_SMALL_KEY",
"OOT_MAGIC_JAR_SMALL",
"OOT_MAGIC_JAR_LARGE",
"OOT_WALLET2",
"OOT_WALLET3",
"OOT_WEIRD_EGG",
"OOT_RECOVERY_HEART",
"OOT_ARROWS_5",
"OOT_ARROWS_10",
"OOT_ARROWS_30",
"OOT_RUPEE_GREEN",
"OOT_RUPEE_BLUE",
"OOT_RUPEE_RED",
"OOT_HEART_CONTAINER2",
"OOT_MILK",
"OOT_MASK_GORON",
"OOT_MASK_ZORA",
"OOT_MASK_GERUDO",
"OOT_GORON_BRACELET",
"OOT_RUPEE_PURPLE",
"OOT_RUPEE_HUGE",
"OOT_SWORD_BIGGORON",
"OOT_ARROW_FIRE",
"OOT_ARROW_ICE",
"OOT_ARROW_LIGHT",
"OOT_GS_TOKEN",
"OOT_SPELL_FIRE",
"OOT_SPELL_WIND",
"OOT_SPELL_LOVE",
"OOT_BULLET_BAG",
"OOT_BULLET_BAG2",
"OOT_STICKS_5",
"OOT_STICKS_10",
"OOT_NUTS_5_ALT",
"OOT_NUTS_10",
"OOT_BOMB",
"OOT_BOMBS_10",
"OOT_BOMBS_20",
"OOT_BOMBS_30",
"OOT_DEKU_SEEDS_30",
"OOT_BOMBCHU_5",
"OOT_BOMBCHU_20",
"OOT_FISH",
"OOT_BUGS",
"OOT_BLUE_FIRE",
"OOT_POE",
"OOT_BIG_POE",
"OOT_TC_SMALL_KEY",
"OOT_TC_RUPEE_GREEN",
"OOT_TC_RUPEE_BLUE",
"OOT_TC_RUPEE_RED",
"OOT_TC_RUPEE_PURPLE",
"OOT_TC_HEART_PIECE",
"OOT_STICK_UPGRADE2",
"OOT_STICK_UPGRADE3",
"OOT_NUT_UPGRADE2",
"OOT_NUT_UPGRADE3",
"OOT_BULLET_BAG3",
"OOT_UNUSED_ICE_TRAP",
"OOT_UNK_7D",
"OOT_UNK_7E",
"OOT_UNK_7F",
"OOT_STICK_UPGRADE",
"OOT_NUT_UPGRADE",
"OOT_RUPEE_RAINBOW",
"OOT_SONG_TP_FOREST",
"OOT_SONG_TP_FIRE",
"OOT_SONG_TP_WATER",
"OOT_SONG_TP_SPIRIT",
"OOT_SONG_TP_SHADOW",
"OOT_SONG_TP_LIGHT",
"OOT_SONG_ZELDA",
"OOT_SONG_EPONA",
"OOT_SONG_SARIA",
"OOT_SONG_SUN",
"OOT_SONG_TIME",
"OOT_SONG_OF_STORMS",
"OOT_SONG_EMPTINESS",
"OOT_SONG_HEALING",
"OOT_SONG_SOARING",
"OOT_SONG_AWAKENING",
"OOT_SONG_GORON",
"OOT_SONG_GORON_HALF",
"OOT_SONG_ZORA",
"OOT_SONG_ORDER",
"OOT_SONG_NOTE_TP_FOREST",
"OOT_SONG_NOTE_TP_FIRE",
"OOT_SONG_NOTE_TP_WATER",
"OOT_SONG_NOTE_TP_SPIRIT",
"OOT_SONG_NOTE_TP_SHADOW",
"OOT_SONG_NOTE_TP_LIGHT",
"OOT_SONG_NOTE_ZELDA",
"OOT_SONG_NOTE_EPONA",
"OOT_SONG_NOTE_SARIA",
"OOT_SONG_NOTE_SUN",
"OOT_SONG_NOTE_TIME",
"OOT_SONG_NOTE_STORMS",
"OOT_SONG_NOTE_EMPTINESS",
"OOT_SONG_NOTE_HEALING",
"OOT_SONG_NOTE_SOARING",
"OOT_SONG_NOTE_AWAKENING",
"OOT_SONG_NOTE_GORON",
"OOT_SONG_NOTE_ZORA",
"OOT_SONG_NOTE_ORDER",
"OOT_SWORD_MASTER",
"OOT_STONE_EMERALD",
"OOT_STONE_RUBY",
"OOT_STONE_SAPPHIRE",
"OOT_MEDALLION_FOREST",
"OOT_MEDALLION_FIRE",
"OOT_MEDALLION_WATER",
"OOT_MEDALLION_SPIRIT",
"OOT_MEDALLION_SHADOW",
"OOT_MEDALLION_LIGHT",
"OOT_MAGIC_UPGRADE",
"OOT_MAGIC_UPGRADE2",
"OOT_DEFENSE_UPGRADE",
"OOT_PROGRESSIVE_SHIELD_DEKU",
"OOT_PROGRESSIVE_SHIELD_HYLIAN",
"OOT_SMALL_KEY_FOREST",
"OOT_SMALL_KEY_FIRE",
"OOT_SMALL_KEY_WATER",
"OOT_SMALL_KEY_SPIRIT",
"OOT_SMALL_KEY_SHADOW",
"OOT_SMALL_KEY_GANON",
"OOT_SMALL_KEY_BOTW",
"OOT_SMALL_KEY_GF",
"OOT_SMALL_KEY_GTG",
"OOT_BOSS_KEY_FOREST",
"OOT_BOSS_KEY_FIRE",
"OOT_BOSS_KEY_WATER",
"OOT_BOSS_KEY_SPIRIT",
"OOT_BOSS_KEY_SHADOW",
"OOT_BOSS_KEY_GANON",
"OOT_MAP_DT",
"OOT_MAP_DC",
"OOT_MAP_JJ",
"OOT_MAP_FOREST",
"OOT_MAP_FIRE",
"OOT_MAP_WATER",
"OOT_MAP_SPIRIT",
"OOT_MAP_SHADOW",
"OOT_MAP_BOTW",
"OOT_MAP_IC",
"OOT_COMPASS_DT",
"OOT_COMPASS_DC",
"OOT_COMPASS_JJ",
"OOT_COMPASS_FOREST",
"OOT_COMPASS_FIRE",
"OOT_COMPASS_WATER",
"OOT_COMPASS_SPIRIT",
"OOT_COMPASS_SHADOW",
"OOT_COMPASS_BOTW",
"OOT_COMPASS_IC",
"OOT_WALLET",
"OOT_WALLET4",
"OOT_TRIFORCE",
"OOT_WALLET5",
"OOT_RUPEE_SILVER_DC",
"OOT_RUPEE_SILVER_BOTW",
"OOT_RUPEE_SILVER_SPIRIT_CHILD",
"OOT_RUPEE_SILVER_SPIRIT_SUN",
"OOT_RUPEE_SILVER_SPIRIT_BOULDERS",
"OOT_RUPEE_SILVER_SPIRIT_LOBBY",
"OOT_RUPEE_SILVER_SPIRIT_ADULT",
"OOT_RUPEE_SILVER_SHADOW_SCYTHE",
"OOT_RUPEE_SILVER_SHADOW_PIT",
"OOT_RUPEE_SILVER_SHADOW_SPIKES",
"OOT_RUPEE_SILVER_SHADOW_BLADES",
"OOT_RUPEE_SILVER_IC_SCYTHE",
"OOT_RUPEE_SILVER_IC_BLOCK",
"OOT_RUPEE_SILVER_GTG_SLOPES",
"OOT_RUPEE_SILVER_GTG_LAVA",
"OOT_RUPEE_SILVER_GTG_WATER",
"OOT_RUPEE_SILVER_GANON_SPIRIT",
"OOT_RUPEE_SILVER_GANON_LIGHT",
"OOT_RUPEE_SILVER_GANON_FIRE",
"OOT_RUPEE_SILVER_GANON_FOREST",
"OOT_RUPEE_SILVER_GANON_SHADOW",
"OOT_RUPEE_SILVER_GANON_WATER",
"OOT_SOUL_ENEMY_STALFOS",
"OOT_SOUL_ENEMY_OCTOROK",
"OOT_SOUL_ENEMY_WALLMASTER",
"OOT_SOUL_ENEMY_DODONGO",
"OOT_SOUL_ENEMY_KEESE",
"OOT_SOUL_ENEMY_TEKTITE",
"OOT_SOUL_ENEMY_LEEVER",
"OOT_SOUL_ENEMY_PEAHAT",
"OOT_SOUL_ENEMY_LIZALFOS_DINOLFOS",
"OOT_SOUL_ENEMY_GOHMA_LARVA",
"OOT_SOUL_ENEMY_SHABOM",
"OOT_SOUL_ENEMY_BABY_DODONGO",
"OOT_SOUL_ENEMY_DARK_LINK",
"OOT_SOUL_ENEMY_BIRI_BARI",
"OOT_SOUL_ENEMY_TAILPASARN",
"OOT_SOUL_ENEMY_PARASITE",
"OOT_SOUL_ENEMY_SKULLTULA",
"OOT_SOUL_ENEMY_TORCH_SLUG",
"OOT_SOUL_ENEMY_MOBLIN",
"OOT_SOUL_ENEMY_ARMOS",
"OOT_SOUL_ENEMY_DEKU_BABA",
"OOT_SOUL_ENEMY_DEKU_SCRUB",
"OOT_SOUL_ENEMY_BUBBLE",
"OOT_SOUL_ENEMY_BEAMOS",
"OOT_SOUL_ENEMY_FLOORMASTER",
"OOT_SOUL_ENEMY_REDEAD_GIBDO",
"OOT_SOUL_ENEMY_SKULLWALLTULA",
"OOT_SOUL_ENEMY_FLARE_DANCER",
"OOT_SOUL_ENEMY_DEAD_HAND",
"OOT_SOUL_ENEMY_SHELL_BLADE",
"OOT_SOUL_ENEMY_LIKE_LIKE",
"OOT_SOUL_ENEMY_SPIKE",
"OOT_SOUL_ENEMY_ANUBIS",
"OOT_SOUL_ENEMY_IRON_KNUCKLE",
"OOT_SOUL_ENEMY_SKULL_KID",
"OOT_SOUL_ENEMY_FLYING_POT",
"OOT_SOUL_ENEMY_FREEZARD",
"OOT_SOUL_ENEMY_STINGER",
"OOT_SOUL_ENEMY_WOLFOS",
"OOT_SOUL_ENEMY_STALCHILD",
"OOT_SOUL_ENEMY_GUAY",
"OOT_SOUL_ENEMY_THIEVES",
"OOT_SOUL_ENEMY_POE",
"OOT_SOUL_BOSS_QUEEN_GOHMA",
"OOT_SOUL_BOSS_KING_DODONGO",
"OOT_SOUL_BOSS_BARINADE",
"OOT_SOUL_BOSS_PHANTOM_GANON",
"OOT_SOUL_BOSS_VOLVAGIA",
"OOT_SOUL_BOSS_MORPHA",
"OOT_SOUL_BOSS_BONGO_BONGO",
"OOT_SOUL_BOSS_TWINROVA",
"OOT_SOUL_NPC_SARIA",
"OOT_SOUL_NPC_DARUNIA",
"OOT_SOUL_NPC_RUTO",
"OOT_SOUL_NPC_KING_ZORA",
"OOT_SOUL_NPC_ZELDA",
"OOT_SOUL_NPC_SHEIK",
"OOT_SOUL_NPC_ANJU",
"OOT_SOUL_NPC_CARPENTERS",
"OOT_SOUL_NPC_GURU_GURU",
"OOT_SOUL_NPC_MIDO",
"OOT_SOUL_NPC_KOKIRI",
"OOT_SOUL_NPC_KOKIRI_SHOPKEEPER",
"OOT_SOUL_NPC_HYLIAN_GUARD",
"OOT_SOUL_NPC_CITIZEN",
"OOT_SOUL_NPC_MALON",
"OOT_SOUL_NPC_TALON",
"OOT_SOUL_NPC_FISHING_POND_OWNER",
"OOT_SOUL_NPC_GORON",
"OOT_SOUL_NPC_MEDIGORON",
"OOT_SOUL_NPC_BIGGORON",
"OOT_SOUL_NPC_GORON_CHILD",
"OOT_SOUL_NPC_GORON_SHOPKEEPER",
"OOT_SOUL_NPC_ZORA",
"OOT_SOUL_NPC_ZORA_SHOPKEEPER",
"OOT_SOUL_NPC_BAZAAR_SHOPKEEPER",
"OOT_SOUL_NPC_BOMBCHU_SHOPKEEPER",
"OOT_SOUL_NPC_POTION_SHOPKEEPER",
"OOT_SOUL_NPC_BOMBCHU_BOWLING_LADY",
"OOT_SOUL_NPC_CHEST_GAME_OWNER",
"OOT_SOUL_NPC_SHOOTING_GALLERY_OWNER",
"OOT_SOUL_NPC_DAMPE",
"OOT_SOUL_NPC_BOMBERS",
"OOT_SOUL_NPC_POE_COLLECTOR",
"OOT_SOUL_NPC_COMPOSER_BROS",
"OOT_SOUL_NPC_HONEY_DARLING",
"OOT_SOUL_NPC_ASTRONOMER",
"OOT_SOUL_NPC_ROOFTOP_MAN",
"OOT_SOUL_NPC_BEAN_SALESMAN",
"OOT_SOUL_NPC_SCIENTIST",
"OOT_SOUL_NPC_GORMAN",
"OOT_SOUL_NPC_GROG",
"OOT_SOUL_NPC_DOG_LADY",
"OOT_SOUL_NPC_CARPET_MAN",
"OOT_SOUL_NPC_OLD_HAG",
"OOT_SOUL_NPC_BANKER",
"OOT_SOUL_NPC_THIEVES",
"OOT_SOUL_ANIMAL_CUCCO",
"OOT_SOUL_ANIMAL_COW",
"OOT_SOUL_ANIMAL_DOG",
"OOT_SOUL_ANIMAL_BUTTERFLY",
"OOT_SOUL_MISC_GS",
"OOT_SOUL_MISC_BUSINESS_SCRUB",
"OOT_KEY_RING",
"OOT_KEY_RING_FOREST",
"OOT_KEY_RING_FIRE",
"OOT_KEY_RING_WATER",
"OOT_KEY_RING_SPIRIT",
"OOT_KEY_RING_SHADOW",
"OOT_KEY_RING_GANON",
"OOT_KEY_RING_BOTW",
"OOT_KEY_RING_GF",
"OOT_KEY_RING_GTG",
"OOT_SKELETON_KEY",
"OOT_COIN_RED",
"OOT_COIN_GREEN",
"OOT_COIN_BLUE",
"OOT_COIN_YELLOW",
"OOT_TRIFORCE_POWER",
"OOT_TRIFORCE_COURAGE",
"OOT_TRIFORCE_WISDOM",
"OOT_POUCH_SILVER_DC",
"OOT_POUCH_SILVER_BOTW",
"OOT_POUCH_SILVER_SPIRIT_CHILD",
"OOT_POUCH_SILVER_SPIRIT_SUN",
"OOT_POUCH_SILVER_SPIRIT_BOULDERS",
"OOT_POUCH_SILVER_SPIRIT_LOBBY",
"OOT_POUCH_SILVER_SPIRIT_ADULT",
"OOT_POUCH_SILVER_SHADOW_SCYTHE",
"OOT_POUCH_SILVER_SHADOW_PIT",
"OOT_POUCH_SILVER_SHADOW_SPIKES",
"OOT_POUCH_SILVER_SHADOW_BLADES",
"OOT_POUCH_SILVER_IC_SCYTHE",
"OOT_POUCH_SILVER_IC_BLOCK",
"OOT_POUCH_SILVER_GTG_SLOPES",
"OOT_POUCH_SILVER_GTG_LAVA",
"OOT_POUCH_SILVER_GTG_WATER",
"OOT_POUCH_SILVER_GANON_SPIRIT",
"OOT_POUCH_SILVER_GANON_LIGHT",
"OOT_POUCH_SILVER_GANON_FIRE",
"OOT_POUCH_SILVER_GANON_FOREST",
"OOT_POUCH_SILVER_GANON_SHADOW",
"OOT_POUCH_SILVER_GANON_WATER",
"OOT_RUPEE_MAGICAL",
"OOT_BUTTON_A",
"OOT_BUTTON_C_RIGHT",
"OOT_BUTTON_C_LEFT",
"OOT_BUTTON_C_UP",
"OOT_BUTTON_C_DOWN",
"OOT_SMALL_KEY_TCG",
"OOT_KEY_RING_TCG",
"OOT_BOTTLE_POTION_RED",
"OOT_BOTTLE_POTION_GREEN",
"OOT_BOTTLE_POTION_BLUE",
"OOT_BOTTLE_FAIRY",
"OOT_BOTTLE_POE",
"OOT_BOTTLE_BIG_POE",
"OOT_BOTTLE_BLUE_FIRE",
"OOT_FISHING_POND_CHILD_FISH_2LBS",
"OOT_FISHING_POND_CHILD_FISH_3LBS",
"OOT_FISHING_POND_CHILD_FISH_4LBS",
"OOT_FISHING_POND_CHILD_FISH_5LBS",
"OOT_FISHING_POND_CHILD_FISH_6LBS",
"OOT_FISHING_POND_CHILD_FISH_7LBS",
"OOT_FISHING_POND_CHILD_FISH_8LBS",
"OOT_FISHING_POND_CHILD_FISH_9LBS",
"OOT_FISHING_POND_CHILD_FISH_10LBS",
"OOT_FISHING_POND_CHILD_FISH_11LBS",
"OOT_FISHING_POND_CHILD_FISH_12LBS",
"OOT_FISHING_POND_CHILD_FISH_13LBS",
"OOT_FISHING_POND_CHILD_FISH_14LBS",
"OOT_FISHING_POND_ADULT_FISH_4LBS",
"OOT_FISHING_POND_ADULT_FISH_5LBS",
"OOT_FISHING_POND_ADULT_FISH_6LBS",
"OOT_FISHING_POND_ADULT_FISH_7LBS",
"OOT_FISHING_POND_ADULT_FISH_8LBS",
"OOT_FISHING_POND_ADULT_FISH_9LBS",
"OOT_FISHING_POND_ADULT_FISH_10LBS",
"OOT_FISHING_POND_ADULT_FISH_11LBS",
"OOT_FISHING_POND_ADULT_FISH_12LBS",
"OOT_FISHING_POND_ADULT_FISH_13LBS",
"OOT_FISHING_POND_ADULT_FISH_14LBS",
"OOT_FISHING_POND_ADULT_FISH_15LBS",
"OOT_FISHING_POND_ADULT_FISH_16LBS",
"OOT_FISHING_POND_ADULT_FISH_17LBS",
"OOT_FISHING_POND_ADULT_FISH_18LBS",
"OOT_FISHING_POND_ADULT_FISH_19LBS",
"OOT_FISHING_POND_ADULT_FISH_20LBS",
"OOT_FISHING_POND_ADULT_FISH_21LBS",
"OOT_FISHING_POND_ADULT_FISH_22LBS",
"OOT_FISHING_POND_ADULT_FISH_23LBS",
"OOT_FISHING_POND_ADULT_FISH_24LBS",
"OOT_FISHING_POND_ADULT_FISH_25LBS",
"OOT_FISHING_POND_CHILD_LOACH_14LBS",
"OOT_FISHING_POND_CHILD_LOACH_15LBS",
"OOT_FISHING_POND_CHILD_LOACH_16LBS",
"OOT_FISHING_POND_CHILD_LOACH_17LBS",
"OOT_FISHING_POND_CHILD_LOACH_18LBS",
"OOT_FISHING_POND_CHILD_LOACH_19LBS",
"OOT_FISHING_POND_ADULT_LOACH_29LBS",
"OOT_FISHING_POND_ADULT_LOACH_30LBS",
"OOT_FISHING_POND_ADULT_LOACH_31LBS",
"OOT_FISHING_POND_ADULT_LOACH_32LBS",
"OOT_FISHING_POND_ADULT_LOACH_33LBS",
"OOT_FISHING_POND_ADULT_LOACH_34LBS",
"OOT_FISHING_POND_ADULT_LOACH_35LBS",
"OOT_FISHING_POND_ADULT_LOACH_36LBS",
"OOT_BOMBCHU_BAG",
"OOT_BOMBCHU_BAG2",
"OOT_BOMBCHU_BAG3",
"OOT_BOMBCHU_BAG_FIRST_5",
"OOT_BOMBCHU_BAG_FIRST_10",
"OOT_BOMBCHU_BAG_FIRST_20",
"OOT_SWORD_RAZOR",
"OOT_SWORD_GILDED",
"OOT_SPIN_UPGRADE",
"OOT_TRIFORCE_FULL",
"OOT_FAIRY_BIG",
"OOT_MASK_BLAST",
"OOT_MASK_STONE",
"OOT_MASK_KAMARO",
"OOT_SCALE_BRONZE",
"OOT_GREAT_FAIRY_SWORD",
"OOT_WEIRD_MUSHROOM",
"OOT_CHATEAU",
"OOT_GOLD_DUST",
"OOT_SEAHORSE2",
"OOT_BOTTLED_GOLD_DUST",
"OOT_BOTTLE_CHATEAU",
"OOT_POWDER_KEG",
"OOT_TRAP_ICE",
"OOT_TRAP_FIRE",
"OOT_TRAP_SHOCK",
"OOT_TRAP_DRAIN",
"OOT_TRAP_ANTI_MAGIC",
"OOT_TRAP_KNOCKBACK",
"OOT_TRAP_RUPOOR",
"OOT_PLATINUM_TOKEN",
"OOT_RUSTY_KEY_TREASURE_CHEST_GAME",
"OOT_RUSTY_KEY_GUARD_HOUSE",
"OOT_RUSTY_KEY_HYRULE_CASTLE",
"OOT_RUSTY_KEY_DOG_LADY_HOUSE",
"OOT_RUSTY_KEY_BACK_ALLEY_HOUSE",
"OOT_RUSTY_KEY_BOMBCHU_SHOP",
"OOT_RUSTY_KEY_MASK_SHOP",
"OOT_RUSTY_KEY_CHILD_BAZAAR",
"OOT_RUSTY_KEY_CHILD_POTION_SHOP",
"OOT_RUSTY_KEY_CHILD_SHOOTING_GALLERY",
"OOT_RUSTY_KEY_BOMBCHU_BOWLING",
"OOT_RUSTY_KEY_LABORATORY",
"OOT_RUSTY_KEY_FISHING_POND",
"OOT_RUSTY_KEY_SILO",
"OOT_RUSTY_KEY_RANCH_STABLE",
"OOT_RUSTY_KEY_RANCH_HOUSE",
"OOT_RUSTY_KEY_RANCH_HOUSE_ROOM",
"OOT_RUSTY_KEY_GRAVEYARD",
"OOT_RUSTY_KEY_WINDMILL",
"OOT_RUSTY_KEY_IMPA_HOUSE",
"OOT_RUSTY_KEY_CARPENTER_HOUSE",
"OOT_RUSTY_KEY_GRANNY_POTION_SHOP",
"OOT_RUSTY_KEY_ADULT_SHOOTING_GALLERY",
"OOT_RUSTY_KEY_SKULLTULA_HOUSE",
"OOT_RUSTY_KEY_ADULT_BAZAAR",
"OOT_RUSTY_KEY_ADULT_POTION_SHOP",
"OOT_RUSTY_KEY_ADULT_POTION_SHOP_BACK",
"OOT_CLOCK",
"OOT_SHOVEL",
"MM_RUPEE_GREEN",
"MM_RUPEE_BLUE",
"MM_RUPEE_RED_10",
"MM_RUPEE_RED",
"MM_RUPEE_PURPLE",
"MM_RUPEE_SILVER",
"MM_RUPEE_GOLD",
"MM_WALLET2",
"MM_WALLET3",
"MM_RECOVERY_HEART",
"MM_HEART_PIECE",
"MM_HEART_CONTAINER",
"MM_MAGIC_JAR_SMALL",
"MM_MAGIC_JAR_LARGE",
"MM_STRAY_FAIRY",
"MM_STRAY_FAIRY_WF",
"MM_STRAY_FAIRY_SH",
"MM_STRAY_FAIRY_GB",
"MM_STRAY_FAIRY_ST",
"MM_STRAY_FAIRY_TOWN",
"MM_TRANSCENDENT_FAIRY",
"MM_BOMB",
"MM_BOMBS_5",
"MM_BOMBS_10",
"MM_BOMBS_20",
"MM_BOMBS_30",
"MM_STICK",
"MM_BOMBCHU_10",
"MM_BOMB_BAG",
"MM_BOMB_BAG2",
"MM_BOMB_BAG3",
"MM_ARROWS_10",
"MM_ARROWS_30",
"MM_ARROWS_40",
"MM_UNK_21",
"MM_BOW",
"MM_QUIVER2",
"MM_QUIVER3",
"MM_ARROW_FIRE",
"MM_ARROW_ICE",
"MM_ARROW_LIGHT",
"MM_NUT",
"MM_NUTS_5",
"MM_NUTS_10",
"MM_UNK_2B",
"MM_UNK_2C",
"MM_UNK_2D",
"MM_BOMBCHU_20",
"MM_UNK_2F",
"MM_UNK_30",
"MM_UNK_31",
"MM_SHIELD_DEKU",
"MM_SHIELD_HERO",
"MM_SHIELD_MIRROR",
"MM_PROGRESSIVE_SHIELD_DEKU",
"MM_PROGRESSIVE_SHIELD_HERO",
"MM_POWDER_KEG",
"MM_MAGIC_BEAN",
"MM_BOMBCHU",
"MM_SWORD_KOKIRI",
"MM_SWORD_RAZOR",
"MM_SWORD_GILDED",
"MM_BOMBCHU_5",
"MM_GREAT_FAIRY_SWORD",
"MM_SMALL_KEY",
"MM_BOSS_KEY",
"MM_MAP",
"MM_COMPASS",
"MM_HOOKSHOT",
"MM_LENS",
"MM_PICTOGRAPH_BOX",
"MM_OCARINA_OF_TIME",
"MM_BOMBER_NOTEBOOK",
"MM_BOTTLE_POTION_RED",
"MM_BOTTLE_EMPTY",
"MM_POTION_RED",
"MM_POTION_GREEN",
"MM_POTION_BLUE",
"MM_FAIRY",
"MM_BOTTLE_MILK",
"MM_FISH",
"MM_BUGS",
"MM_BIG_POE",
"MM_BOTTLED_GOLD_DUST",
"MM_WEIRD_MUSHROOM",
"MM_BOTTLE_CHATEAU",
"MM_MASK_DEKU",
"MM_MASK_GORON",
"MM_MASK_ZORA",
"MM_MASK_FIERCE_DEITY",
"MM_MASK_CAPTAIN",
"MM_MASK_GIANT",
"MM_MASK_ALL_NIGHT",
"MM_MASK_BUNNY",
"MM_MASK_KEATON",
"MM_MASK_GARO",
"MM_MASK_ROMANI",
"MM_MASK_TROUPE_LEADER",
"MM_MASK_POSTMAN",
"MM_MASK_COUPLE",
"MM_MASK_GREAT_FAIRY",
"MM_MASK_GIBDO",
"MM_MASK_DON_GERO",
"MM_MASK_KAMARO",
"MM_MASK_TRUTH",
"MM_MASK_STONE",
"MM_MASK_BREMEN",
"MM_MASK_BLAST",
"MM_MASK_SCENTS",
"MM_MASK_KAFEI",
"MM_MASK_GERUDO",
"MM_MASK_SKULL",
"MM_MASK_SPOOKY",
"MM_MASK_ADULT",
"MM_CHATEAU",
"MM_MILK",
"MM_GOLD_DUST",
"MM_SEAHORSE2",
"MM_MOON_TEAR",
"MM_DEED_LAND",
"MM_DEED_SWAMP",
"MM_DEED_MOUNTAIN",
"MM_DEED_OCEAN",
"MM_ROOM_KEY",
"MM_LETTER_TO_MAMA",
"MM_LETTER_TO_KAFEI",
"MM_PENDANT_OF_MEMORIES",
"MM_WORLD_MAP_CLOCK_TOWN",
"MM_WORLD_MAP_WOODFALL",
"MM_WORLD_MAP_SNOWHEAD",
"MM_WORLD_MAP_ROMANI_RANCH",
"MM_WORLD_MAP_GREAT_BAY",
"MM_WORLD_MAP_STONE_TOWER",
"MM_MAGIC_UPGRADE",
"MM_MAGIC_UPGRADE2",
"MM_DEFENSE_UPGRADE",
"MM_SPIN_UPGRADE",
"MM_SONG_AWAKENING",
"MM_SONG_GORON",
"MM_SONG_ZORA",
"MM_SONG_EMPTINESS",
"MM_SONG_ORDER",
"MM_SONG_TIME",
"MM_SONG_HEALING",
"MM_SONG_EPONA",
"MM_SONG_SOARING",
"MM_SONG_OF_STORMS",
"MM_SONG_SUN",
"MM_SONG_TP_FOREST",
"MM_SONG_TP_FIRE",
"MM_SONG_TP_WATER",
"MM_SONG_TP_SPIRIT",
"MM_SONG_TP_SHADOW",
"MM_SONG_TP_LIGHT",
"MM_SONG_ZELDA",
"MM_SONG_SARIA",
"MM_SONG_GORON_HALF",
"MM_SONG_NOTE_AWAKENING",
"MM_SONG_NOTE_GORON",
"MM_SONG_NOTE_ZORA",
"MM_SONG_NOTE_EMPTINESS",
"MM_SONG_NOTE_ORDER",
"MM_SONG_NOTE_TIME",
"MM_SONG_NOTE_HEALING",
"MM_SONG_NOTE_EPONA",
"MM_SONG_NOTE_SOARING",
"MM_SONG_NOTE_STORMS",
"MM_SONG_NOTE_SUN",
"MM_SONG_NOTE_TP_FOREST",
"MM_SONG_NOTE_TP_FIRE",
"MM_SONG_NOTE_TP_WATER",
"MM_SONG_NOTE_TP_SPIRIT",
"MM_SONG_NOTE_TP_SHADOW",
"MM_SONG_NOTE_TP_LIGHT",
"MM_SONG_NOTE_ZELDA",
"MM_SONG_NOTE_SARIA",
"MM_REMAINS_ODOLWA",
"MM_REMAINS_GOHT",
"MM_REMAINS_GYORG",
"MM_REMAINS_TWINMOLD",
"MM_GS_TOKEN_SWAMP",
"MM_GS_TOKEN_OCEAN",
"MM_PLATINUM_TOKEN",
"MM_SMALL_KEY_WF",
"MM_SMALL_KEY_SH",
"MM_SMALL_KEY_GB",
"MM_SMALL_KEY_ST",
"MM_BOSS_KEY_WF",
"MM_BOSS_KEY_SH",
"MM_BOSS_KEY_GB",
"MM_BOSS_KEY_ST",
"MM_MAP_WF",
"MM_MAP_SH",
"MM_MAP_GB",
"MM_MAP_ST",
"MM_COMPASS_WF",
"MM_COMPASS_SH",
"MM_COMPASS_GB",
"MM_COMPASS_ST",
"MM_OCARINA_FAIRY",
"MM_HOOKSHOT_SHORT",
"MM_WALLET",
"MM_WALLET4",
"MM_OWL_GREAT_BAY",
"MM_OWL_ZORA_CAPE",
"MM_OWL_SNOWHEAD",
"MM_OWL_MOUNTAIN_VILLAGE",
"MM_OWL_CLOCK_TOWN",
"MM_OWL_MILK_ROAD",
"MM_OWL_WOODFALL",
"MM_OWL_SOUTHERN_SWAMP",
"MM_OWL_IKANA_CANYON",
"MM_OWL_STONE_TOWER",
"MM_OWL_HIDDEN",
"MM_WALLET5",
"MM_SLINGSHOT",
"MM_DEKU_SEEDS_5",
"MM_DEKU_SEEDS_30",
"MM_BULLET_BAG2",
"MM_BULLET_BAG3",
"MM_SOUL_ENEMY_OCTOROK",
"MM_SOUL_ENEMY_WALLMASTER",
"MM_SOUL_ENEMY_DODONGO",
"MM_SOUL_ENEMY_KEESE",
"MM_SOUL_ENEMY_TEKTITE",
"MM_SOUL_ENEMY_PEAHAT",
"MM_SOUL_ENEMY_LIZALFOS_DINOLFOS",
"MM_SOUL_ENEMY_SKULLTULA",
"MM_SOUL_ENEMY_ARMOS",
"MM_SOUL_ENEMY_DEKU_BABA",
"MM_SOUL_ENEMY_DEKU_SCRUB",
"MM_SOUL_ENEMY_BUBBLE",
"MM_SOUL_ENEMY_BEAMOS",
"MM_SOUL_ENEMY_REDEAD_GIBDO",
"MM_SOUL_ENEMY_SKULLWALLTULA",
"MM_SOUL_ENEMY_SHELL_BLADE",
"MM_SOUL_ENEMY_LIKE_LIKE",
"MM_SOUL_ENEMY_IRON_KNUCKLE",
"MM_SOUL_ENEMY_FREEZARD",
"MM_SOUL_ENEMY_WOLFOS",
"MM_SOUL_ENEMY_GUAY",
"MM_SOUL_ENEMY_FLYING_POT",
"MM_SOUL_ENEMY_FLOORMASTER",
"MM_SOUL_ENEMY_CHUCHU",
"MM_SOUL_ENEMY_DEEP_PYTHON",
"MM_SOUL_ENEMY_STALCHILD",
"MM_SOUL_ENEMY_LEEVER",
"MM_SOUL_ENEMY_SKULLFISH",
"MM_SOUL_ENEMY_DEXIHAND",
"MM_SOUL_ENEMY_DRAGONFLY",
"MM_SOUL_ENEMY_EENO",
"MM_SOUL_ENEMY_EYEGORE",
"MM_SOUL_ENEMY_HIPLOOP",
"MM_SOUL_ENEMY_REAL_BOMBCHU",
"MM_SOUL_ENEMY_TAKKURI",
"MM_SOUL_ENEMY_BOE",
"MM_SOUL_ENEMY_NEJIRON",
"MM_SOUL_ENEMY_BIO_BABA",
"MM_SOUL_ENEMY_GARO",
"MM_SOUL_ENEMY_WIZZROBE",
"MM_SOUL_ENEMY_GOMESS",
"MM_SOUL_ENEMY_GEKKO",
"MM_SOUL_ENEMY_BAD_BAT",
"MM_SOUL_ENEMY_SNAPPER",
"MM_SOUL_ENEMY_WART",
"MM_SOUL_ENEMY_CAPTAIN_KEETA",
"MM_SOUL_ENEMY_THIEVES",
"MM_SOUL_ENEMY_POE",
"MM_SOUL_BOSS_ODOLWA",
"MM_SOUL_BOSS_GOHT",
"MM_SOUL_BOSS_GYORG",
"MM_SOUL_BOSS_TWINMOLD",
"MM_SOUL_BOSS_IGOS",
"MM_SOUL_NPC_AROMA",
"MM_SOUL_NPC_SHOOTING_GALLERY_OWNER",
"MM_SOUL_NPC_BAZAAR_SHOPKEEPER",
"MM_SOUL_NPC_GORON",
"MM_SOUL_NPC_GORON_CHILD",
"MM_SOUL_NPC_BOMBCHU_SHOPKEEPER",
"MM_SOUL_NPC_BOMBERS",
"MM_SOUL_NPC_BUTLER_DEKU",
"MM_SOUL_NPC_CITIZEN",
"MM_SOUL_NPC_COMPOSER_BROS",
"MM_SOUL_NPC_DAMPE",
"MM_SOUL_NPC_DEKU_KING",
"MM_SOUL_NPC_DEKU_PRINCESS",
"MM_SOUL_NPC_PLAYGROUND_SCRUBS",
"MM_SOUL_NPC_GORON_ELDER",
"MM_SOUL_NPC_CHEST_GAME_OWNER",
"MM_SOUL_NPC_GORON_SHOPKEEPER",
"MM_SOUL_NPC_HONEY_DARLING",
"MM_SOUL_NPC_TOTO",
"MM_SOUL_NPC_RUTO",
"MM_SOUL_NPC_ZORA_MUSICIANS",
"MM_SOUL_NPC_KAFEI",
"MM_SOUL_NPC_KEATON",
"MM_SOUL_NPC_KOUME_KOTAKE",
"MM_SOUL_NPC_MAYOR_DOTOUR",
"MM_SOUL_NPC_MEDIGORON",
"MM_SOUL_NPC_BIGGORON",
"MM_SOUL_NPC_TALON",
"MM_SOUL_NPC_ASTRONOMER",
"MM_SOUL_NPC_POE_COLLECTOR",
"MM_SOUL_NPC_BLACKSMITHS",
"MM_SOUL_NPC_BOMBCHU_BOWLING_LADY",
"MM_SOUL_NPC_TINGLE",
"MM_SOUL_NPC_TOILET_HAND",
"MM_SOUL_NPC_FISHING_POND_OWNER",
"MM_SOUL_NPC_ROOFTOP_MAN",
"MM_SOUL_NPC_ZORA",
"MM_SOUL_NPC_ZORA_SHOPKEEPER",
"MM_SOUL_NPC_MALON",
"MM_SOUL_NPC_BEAN_SALESMAN",
"MM_SOUL_NPC_CARPENTERS",
"MM_SOUL_NPC_ANJU",
"MM_SOUL_NPC_GURU_GURU",
"MM_SOUL_NPC_SCIENTIST",
"MM_SOUL_NPC_GORMAN",
"MM_SOUL_NPC_GROG",
"MM_SOUL_NPC_DOG_LADY",
"MM_SOUL_NPC_CARPET_MAN",
"MM_SOUL_NPC_OLD_HAG",
"MM_SOUL_NPC_TOURIST_CENTER",
"MM_SOUL_NPC_BANKER",
"MM_SOUL_NPC_MOON_CHILDREN",
"MM_SOUL_NPC_THIEVES",
"MM_SOUL_ANIMAL_CUCCO",
"MM_SOUL_ANIMAL_COW",
"MM_SOUL_ANIMAL_DOG",
"MM_SOUL_ANIMAL_BUTTERFLY",
"MM_SOUL_MISC_GS",
"MM_SOUL_MISC_BUSINESS_SCRUB",
"MM_KEY_RING",
"MM_KEY_RING_WF",
"MM_KEY_RING_SH",
"MM_KEY_RING_GB",
"MM_KEY_RING_ST",
"MM_SKELETON_KEY",
"MM_BUTTON_A",
"MM_BUTTON_C_RIGHT",
"MM_BUTTON_C_LEFT",
"MM_BUTTON_C_UP",
"MM_BUTTON_C_DOWN",
"MM_BOTTLE_POTION_GREEN",
"MM_BOTTLE_POTION_BLUE",
"MM_BOTTLE_FAIRY",
"MM_POE",
"MM_BOTTLE_POE",
"MM_BOTTLE_BIG_POE",
"MM_BOMBCHU_BAG",
"MM_BOMBCHU_BAG2",
"MM_BOMBCHU_BAG3",
"MM_BOMBCHU_BAG_FIRST_1",
"MM_BOMBCHU_BAG_FIRST_5",
"MM_BOMBCHU_BAG_FIRST_10",
"MM_BOMBCHU_BAG_FIRST_20",
"MM_FAIRY_BIG",
"MM_SPELL_FIRE",
"MM_SPELL_WIND",
"MM_SPELL_LOVE",
"MM_BOOTS_IRON",
"MM_BOOTS_HOVER",
"MM_TUNIC_GORON",
"MM_TUNIC_ZORA",
"MM_SCALE_BRONZE",
"MM_SCALE_SILVER",
"MM_SCALE_GOLDEN",
"MM_GORON_BRACELET",
"MM_SILVER_GAUNTLETS",
"MM_GOLDEN_GAUNTLETS",
"MM_CLOCK1",
"MM_CLOCK2",
"MM_CLOCK3",
"MM_CLOCK4",
"MM_CLOCK5",
"MM_CLOCK6",
"MM_HAMMER",
"MM_BOOMERANG",
"MM_STICK_UPGRADE",
"MM_STICK_UPGRADE2",
"MM_STICK_UPGRADE3",
"MM_NUT_UPGRADE",
"MM_NUT_UPGRADE2",
"MM_NUT_UPGRADE3",
"MM_STONE_OF_AGONY",
"MM_BOTTLE_RUTO_LETTER",
"MM_BOTTLE_BLUE_FIRE",
"MM_BLUE_FIRE",
"MM_RUSTY_KEY_TOURIST_INFORMATION",
"MM_RUSTY_KEY_POTION_SHOP",
"MM_RUSTY_KEY_POST_OFFICE",
"MM_RUSTY_KEY_SWORDSMAN_SCHOOL",
"MM_RUSTY_KEY_LOTTERY",
"MM_RUSTY_KEY_BOMB_SHOP",
"MM_RUSTY_KEY_TRADING_POST",
"MM_RUSTY_KEY_CURIOSITY_SHOP",
"MM_RUSTY_KEY_KAFEI_HIDEOUT",
"MM_RUSTY_KEY_TOWN_ARCHERY",
"MM_RUSTY_KEY_SWAMP_ARCHERY",
"MM_RUSTY_KEY_OBSERVATORY",
"MM_RUSTY_KEY_BLACKSMITH",
"MM_RUSTY_KEY_MUSIC_HOUSE",
"MM_RUSTY_KEY_LABORATORY",
"MM_RUSTY_KEY_BENEATH_GRAVEYARD",
"MM_RUSTY_KEY_DAMPE_HOUSE",
"MM_RUSTY_KEY_MAYOR_RESIDENCE",
"MM_RUSTY_KEY_MAYOR_RESIDENCE_OFFICE",
"MM_RUSTY_KEY_MAYOR_RESIDENCE_SALON",
"MM_RUSTY_KEY_MAYOR_RESIDENCE_KAFEI",
"MM_RUSTY_KEY_TREASURE_CHEST_GAME",
"MM_RUSTY_KEY_HONEY_DARLING",
"MM_RUSTY_KEY_MILK_BAR",
"MM_RUSTY_KEY_DOG_RACETRACK",
"MM_RUSTY_KEY_CUCCO_SHACK",
"MM_RUSTY_KEY_RANCH_HOUSE",
"MM_RUSTY_KEY_RANCH_BARN",
"MM_RUSTY_KEY_RANCH_HOUSE_ROOM",
"MM_RUSTY_KEY_ZORA_SHOP",
"MM_RUSTY_KEY_ZORA_JAPAS_ROOM",
"MM_RUSTY_KEY_ZORA_TIJO_ROOM",
"MM_RUSTY_KEY_ZORA_LULU_ROOM",
"MM_RUSTY_KEY_ZORA_EVAN_ROOM",
"MM_RUSTY_KEY_STOCK_POT_INN",
"MM_RUSTY_KEY_STOCK_POT_INN_ROOF",
"MM_RUSTY_KEY_GRANDMA_ROOM",
"MM_RUSTY_KEY_STOCK_POT_INN_STAFF_ROOM",
"MM_RUSTY_KEY_STOCK_POT_INN_DORMITORY",
"MM_SHOVEL",
"MM_MASK_MAJORA",
"MM_SOLD_OUT",
"NOTHING",
"MM_SCOOP",

# Extra Adds
"SHARED_ARROW_FIRE",
"SHARED_ARROW_ICE",
"SHARED_ARROW_LIGHT",
"SHARED_ARROWS_10",
"SHARED_ARROWS_30",
"SHARED_ARROWS_40",
"SHARED_ARROWS_5",
"SHARED_BIG_POE",
"SHARED_BLUE_FIRE",
"SHARED_BOMB",
"SHARED_BOMB_BAG",
"SHARED_BOMBCHU",
"SHARED_BOMBCHU_10",
"SHARED_BOMBCHU_20",
"SHARED_BOMBCHU_5",
"SHARED_BOMBCHU_BAG",
"SHARED_BOMBS_10",
"SHARED_BOMBS_20",
"SHARED_BOMBS_30",
"SHARED_BOMBS_5",
"SHARED_BOOTS_HOVER",
"SHARED_BOOTS_IRON",
"SHARED_BOTTLE_BIG_POE",
"SHARED_BOTTLE_BLUE_FIRE",
"SHARED_BOTTLE_CHATEAU",
"SHARED_BOTTLE_EMPTY",
"SHARED_BOTTLE_FAIRY",
"SHARED_BOTTLE_MILK",
"SHARED_BOTTLE_POE",
"SHARED_BOTTLE_POTION_BLUE",
"SHARED_BOTTLE_POTION_GREEN",
"SHARED_BOTTLE_POTION_RED",
"SHARED_BOTTLE_RUTO_LETTER",
"SHARED_BOTTLED_GOLD_DUST",
"SHARED_BOW",
"SHARED_BUGS",
"SHARED_BUTTON_A",
"SHARED_BUTTON_C_DOWN",
"SHARED_BUTTON_C_LEFT",
"SHARED_BUTTON_C_RIGHT",
"SHARED_BUTTON_C_UP",
"SHARED_CHATEAU",
"SHARED_DEFENSE_UPGRADE",
"SHARED_FAIRY",
"SHARED_FAIRY_BIG",
"SHARED_FISH",
"SHARED_HAMMER",
"SHARED_BOOMERANG",
"SHARED_HEART_CONTAINER",
"SHARED_HEART_PIECE",
"SHARED_HOOKSHOT",
"SHARED_LENS",
"SHARED_MAGIC_JAR_LARGE",
"SHARED_MAGIC_JAR_SMALL",
"SHARED_MAGIC_UPGRADE",
"SHARED_MASK_BLAST",
"SHARED_MASK_BUNNY",
"SHARED_MASK_GORON",
"SHARED_MASK_KEATON",
"SHARED_MASK_KAMARO",
"SHARED_MASK_STONE",
"SHARED_MASK_TRUTH",
"SHARED_MASK_ZORA",
"SHARED_MILK",
"SHARED_NUT",
"SHARED_NUT_UPGRADE",
"SHARED_NUTS_10",
"SHARED_NUTS_5",
"SHARED_OCARINA",
"SHARED_PLATINUM_TOKEN",
"SHARED_POE",
"SHARED_POTION_BLUE",
"SHARED_POTION_GREEN",
"SHARED_POTION_RED",
"SHARED_RECOVERY_HEART",
"SHARED_RUPEE_BLUE",
"SHARED_RUPEE_GOLD",
"SHARED_RUPEE_GREEN",
"SHARED_RUPEE_PURPLE",
"SHARED_RUPEE_RAINBOW",
"SHARED_RUPEE_RED",
"SHARED_RUPEE_SILVER",
"SHARED_SCALE",
"SHARED_SHIELD",
"SHARED_SHIELD_DEKU",
"SHARED_SHIELD_HYLIAN",
"SHARED_SHIELD_MIRROR",
"SHARED_SHOVEL",
"SHARED_SKELETON_KEY",
"SHARED_SONG_EMPTINESS",
"SHARED_SONG_EPONA",
"SHARED_SONG_NOTE_EMPTINESS",
"SHARED_SONG_NOTE_EPONA",
"SHARED_SONG_NOTE_STORMS",
"SHARED_SONG_NOTE_SUN",
"SHARED_SONG_NOTE_TIME",
"SHARED_SONG_OF_STORMS",
"SHARED_SONG_SUN",
"SHARED_SONG_TIME",
"SHARED_SONG_NOTE_HEALING",
"SHARED_SONG_NOTE_SOARING",
"SHARED_SONG_NOTE_AWAKENING",
"SHARED_SONG_NOTE_GORON",
"SHARED_SONG_NOTE_ZORA",
"SHARED_SONG_NOTE_ORDER",
"SHARED_SONG_HEALING",
"SHARED_SONG_SOARING",
"SHARED_SONG_AWAKENING",
"SHARED_SONG_GORON",
"SHARED_SONG_GORON_HALF",
"SHARED_SONG_ZORA",
"SHARED_SONG_ORDER",
"SHARED_SONG_NOTE_SARIA",
"SHARED_SONG_NOTE_TP_FIRE",
"SHARED_SONG_NOTE_TP_FOREST",
"SHARED_SONG_NOTE_TP_LIGHT",
"SHARED_SONG_NOTE_TP_SHADOW",
"SHARED_SONG_NOTE_TP_SPIRIT",
"SHARED_SONG_NOTE_TP_WATER",
"SHARED_SONG_NOTE_ZELDA",
"SHARED_SONG_SARIA",
"SHARED_SONG_TP_FIRE",
"SHARED_SONG_TP_FOREST",
"SHARED_SONG_TP_LIGHT",
"SHARED_SONG_TP_SHADOW",
"SHARED_SONG_TP_SPIRIT",
"SHARED_SONG_TP_WATER",
"SHARED_SONG_ZELDA",
"SHARED_SOUL_ANIMAL_BUTTERFLY",
"SHARED_SOUL_ANIMAL_COW",
"SHARED_SOUL_ANIMAL_CUCCO",
"SHARED_SOUL_ANIMAL_DOG",
"SHARED_SOUL_ENEMY_ARMOS",
"SHARED_SOUL_ENEMY_BEAMOS",
"SHARED_SOUL_ENEMY_BUBBLE",
"SHARED_SOUL_ENEMY_DEKU_BABA",
"SHARED_SOUL_ENEMY_DEKU_SCRUB",
"SHARED_SOUL_ENEMY_DODONGO",
"SHARED_SOUL_ENEMY_FLOORMASTER",
"SHARED_SOUL_ENEMY_FLYING_POT",
"SHARED_SOUL_ENEMY_FREEZARD",
"SHARED_SOUL_ENEMY_GUAY",
"SHARED_SOUL_ENEMY_IRON_KNUCKLE",
"SHARED_SOUL_ENEMY_KEESE",
"SHARED_SOUL_ENEMY_LEEVER",
"SHARED_SOUL_ENEMY_LIKE_LIKE",
"SHARED_SOUL_ENEMY_LIZALFOS_DINOLFOS",
"SHARED_SOUL_ENEMY_OCTOROK",
"SHARED_SOUL_ENEMY_PEAHAT",
"SHARED_SOUL_ENEMY_POE",
"SHARED_SOUL_ENEMY_REDEAD_GIBDO",
"SHARED_SOUL_ENEMY_SHELL_BLADE",
"SHARED_SOUL_ENEMY_SKULLTULA",
"SHARED_SOUL_ENEMY_SKULLWALLTULA",
"SHARED_SOUL_ENEMY_STALCHILD",
"SHARED_SOUL_ENEMY_TEKTITE",
"SHARED_SOUL_ENEMY_THIEVES",
"SHARED_SOUL_ENEMY_WALLMASTER",
"SHARED_SOUL_ENEMY_WOLFOS",
"SHARED_SOUL_MISC_BUSINESS_SCRUB",
"SHARED_SOUL_MISC_GS",
"SHARED_SOUL_NPC_ANJU",
"SHARED_SOUL_NPC_ASTRONOMER",
"SHARED_SOUL_NPC_BANKER",
"SHARED_SOUL_NPC_BAZAAR_SHOPKEEPER",
"SHARED_SOUL_NPC_BEAN_SALESMAN",
"SHARED_SOUL_NPC_BIGGORON",
"SHARED_SOUL_NPC_BOMBCHU_BOWLING_LADY",
"SHARED_SOUL_NPC_BOMBCHU_SHOPKEEPER",
"SHARED_SOUL_NPC_BOMBERS",
"SHARED_SOUL_NPC_CARPENTERS",
"SHARED_SOUL_NPC_CARPET_MAN",
"SHARED_SOUL_NPC_CHEST_GAME_OWNER",
"SHARED_SOUL_NPC_CITIZEN",
"SHARED_SOUL_NPC_COMPOSER_BROS",
"SHARED_SOUL_NPC_DAMPE",
"SHARED_SOUL_NPC_DOG_LADY",
"SHARED_SOUL_NPC_FISHING_POND_OWNER",
"SHARED_SOUL_NPC_GORMAN",
"SHARED_SOUL_NPC_GORON",
"SHARED_SOUL_NPC_GORON_CHILD",
"SHARED_SOUL_NPC_GORON_SHOPKEEPER",
"SHARED_SOUL_NPC_GROG",
"SHARED_SOUL_NPC_GURU_GURU",
"SHARED_SOUL_NPC_HONEY_DARLING",
"SHARED_SOUL_NPC_MALON",
"SHARED_SOUL_NPC_MEDIGORON",
"SHARED_SOUL_NPC_OLD_HAG",
"SHARED_SOUL_NPC_POE_COLLECTOR",
"SHARED_SOUL_NPC_ROOFTOP_MAN",
"SHARED_SOUL_NPC_RUTO",
"SHARED_SOUL_NPC_SCIENTIST",
"SHARED_SOUL_NPC_SHOOTING_GALLERY_OWNER",
"SHARED_SOUL_NPC_TALON",
"SHARED_SOUL_NPC_THIEVES",
"SHARED_SOUL_NPC_ZORA",
"SHARED_SOUL_NPC_ZORA_SHOPKEEPER",
"SHARED_SPELL_FIRE",
"SHARED_SPELL_LOVE",
"SHARED_SPELL_WIND",
"SHARED_SPIN_UPGRADE",
"SHARED_STICK",
"SHARED_STICK_UPGRADE",
"SHARED_STICKS_10",
"SHARED_STICKS_5",
"SHARED_STONE_OF_AGONY",
"SHARED_STRENGTH",
"SHARED_SWORD",
"SHARED_TRIFORCE",
"SHARED_TRIFORCE_COURAGE",
"SHARED_TRIFORCE_POWER",
"SHARED_TRIFORCE_WISDOM",
"SHARED_TUNIC_GORON",
"SHARED_TUNIC_ZORA",
"SHARED_WALLET",
"OOT_PROG_WALLET",
"OOT_SWORD",
"OOT_OCARINA",
"OOT_SCALE",
"OOT_SHIELD",
"OOT_STRENGTH",
"OOT_SWORD_GORON",
"MM_PROG_WALLET",
"MM_SWORD",
"MM_CLOCK",
"MM_OCARINA",
"MM_SCALE",
"MM_SHIELD",
"MM_STRENGTH",
"SHARED_SLINGSHOT",
"SHARED_DEKU_SEEDS_5",
"SHARED_DEKU_SEEDS_30",
"SHARED_BULLET_BAG2",
"SHARED_BULLET_BAG3",
"SHARED_QUIVER2",
"SHARED_QUIVER3",
"SHARED_POWDER_KEG",
"SHARED_MASK_GERUDO",
"SHARED_MASK_SKULL",
"SHARED_MASK_SPOOKY",
"SHARED_GREAT_FAIRY_SWORD"
]

# Ordre des items du build STABLE (l'enum tel que rapporte le DLL stable). N'inclut PAS les items
# SHARED_* ni ce qui suit "# Extra Adds" dans itemOrder : ces items sont dev-only et sont tries apres
# tous les items stable, donc ils ne decalent jamais un ID stable. Sert a generer ResolveRawItemID.
stableOrder = [
"OOT_BOMBS_5",
"OOT_NUTS_5",
"OOT_BOMBCHU_10",
"OOT_BOW",
"OOT_SLINGSHOT",
"OOT_BOOMERANG",
"OOT_STICK",
"OOT_HOOKSHOT",
"OOT_LONGSHOT",
"OOT_LENS",
"OOT_ZELDA_LETTER",
"OOT_OCARINA_TIME",
"OOT_HAMMER",
"OOT_COJIRO",
"OOT_BOTTLE_EMPTY",
"OOT_POTION_RED",
"OOT_POTION_GREEN",
"OOT_POTION_BLUE",
"OOT_FAIRY",
"OOT_BOTTLE_MILK",
"OOT_BOTTLE_RUTO_LETTER",
"OOT_MAGIC_BEAN",
"OOT_MASK_SKULL",
"OOT_MASK_SPOOKY",
"OOT_CHICKEN",
"OOT_MASK_KEATON",
"OOT_MASK_BUNNY",
"OOT_MASK_TRUTH",
"OOT_POCKET_EGG",
"OOT_POCKET_CUCCO",
"OOT_ODD_MUSHROOM",
"OOT_ODD_POTION",
"OOT_POACHER_SAW",
"OOT_BROKEN_GORON_SWORD",
"OOT_PRESCRIPTION",
"OOT_EYEBALL_FROG",
"OOT_EYE_DROPS",
"OOT_CLAIM_CHECK",
"OOT_SWORD_KOKIRI",
"OOT_SWORD_KNIFE",
"OOT_SHIELD_DEKU",
"OOT_SHIELD_HYLIAN",
"OOT_SHIELD_MIRROR",
"OOT_TUNIC_GORON",
"OOT_TUNIC_ZORA",
"OOT_BOOTS_IRON",
"OOT_BOOTS_HOVER",
"OOT_QUIVER2",
"OOT_QUIVER3",
"OOT_BOMB_BAG",
"OOT_BOMB_BAG2",
"OOT_BOMB_BAG3",
"OOT_SILVER_GAUNTLETS",
"OOT_GOLDEN_GAUNTLETS",
"OOT_SCALE_SILVER",
"OOT_SCALE_GOLDEN",
"OOT_STONE_OF_AGONY",
"OOT_GERUDO_CARD",
"OOT_OCARINA_FAIRY",
"OOT_DEKU_SEEDS_5",
"OOT_HEART_CONTAINER",
"OOT_HEART_PIECE",
"OOT_BOSS_KEY",
"OOT_COMPASS",
"OOT_MAP",
"OOT_SMALL_KEY",
"OOT_MAGIC_JAR_SMALL",
"OOT_MAGIC_JAR_LARGE",
"OOT_WALLET2",
"OOT_WALLET3",
"OOT_WEIRD_EGG",
"OOT_RECOVERY_HEART",
"OOT_ARROWS_5",
"OOT_ARROWS_10",
"OOT_ARROWS_30",
"OOT_RUPEE_GREEN",
"OOT_RUPEE_BLUE",
"OOT_RUPEE_RED",
"OOT_HEART_CONTAINER2",
"OOT_MILK",
"OOT_MASK_GORON",
"OOT_MASK_ZORA",
"OOT_MASK_GERUDO",
"OOT_GORON_BRACELET",
"OOT_RUPEE_PURPLE",
"OOT_RUPEE_HUGE",
"OOT_SWORD_BIGGORON",
"OOT_ARROW_FIRE",
"OOT_ARROW_ICE",
"OOT_ARROW_LIGHT",
"OOT_GS_TOKEN",
"OOT_SPELL_FIRE",
"OOT_SPELL_WIND",
"OOT_SPELL_LOVE",
"OOT_BULLET_BAG",
"OOT_BULLET_BAG2",
"OOT_STICKS_5",
"OOT_STICKS_10",
"OOT_NUTS_5_ALT",
"OOT_NUTS_10",
"OOT_BOMB",
"OOT_BOMBS_10",
"OOT_BOMBS_20",
"OOT_BOMBS_30",
"OOT_DEKU_SEEDS_30",
"OOT_BOMBCHU_5",
"OOT_BOMBCHU_20",
"OOT_FISH",
"OOT_BUGS",
"OOT_BLUE_FIRE",
"OOT_POE",
"OOT_BIG_POE",
"OOT_TC_SMALL_KEY",
"OOT_TC_RUPEE_GREEN",
"OOT_TC_RUPEE_BLUE",
"OOT_TC_RUPEE_RED",
"OOT_TC_RUPEE_PURPLE",
"OOT_TC_HEART_PIECE",
"OOT_STICK_UPGRADE2",
"OOT_STICK_UPGRADE3",
"OOT_NUT_UPGRADE2",
"OOT_NUT_UPGRADE3",
"OOT_BULLET_BAG3",
"OOT_UNUSED_ICE_TRAP",
"OOT_UNK_7D",
"OOT_UNK_7E",
"OOT_UNK_7F",
"OOT_RUPEE_RAINBOW",
"OOT_SONG_TP_FOREST",
"OOT_SONG_TP_FIRE",
"OOT_SONG_TP_WATER",
"OOT_SONG_TP_SPIRIT",
"OOT_SONG_TP_SHADOW",
"OOT_SONG_TP_LIGHT",
"OOT_SONG_ZELDA",
"OOT_SONG_EPONA",
"OOT_SONG_SARIA",
"OOT_SONG_SUN",
"OOT_SONG_TIME",
"OOT_SONG_OF_STORMS",
"OOT_SONG_EMPTINESS",
"OOT_SONG_NOTE_TP_FOREST",
"OOT_SONG_NOTE_TP_FIRE",
"OOT_SONG_NOTE_TP_WATER",
"OOT_SONG_NOTE_TP_SPIRIT",
"OOT_SONG_NOTE_TP_SHADOW",
"OOT_SONG_NOTE_TP_LIGHT",
"OOT_SONG_NOTE_ZELDA",
"OOT_SONG_NOTE_EPONA",
"OOT_SONG_NOTE_SARIA",
"OOT_SONG_NOTE_SUN",
"OOT_SONG_NOTE_TIME",
"OOT_SONG_NOTE_STORMS",
"OOT_SONG_NOTE_EMPTINESS",
"OOT_SWORD_MASTER",
"OOT_STONE_EMERALD",
"OOT_STONE_RUBY",
"OOT_STONE_SAPPHIRE",
"OOT_MEDALLION_FOREST",
"OOT_MEDALLION_FIRE",
"OOT_MEDALLION_WATER",
"OOT_MEDALLION_SPIRIT",
"OOT_MEDALLION_SHADOW",
"OOT_MEDALLION_LIGHT",
"OOT_MAGIC_UPGRADE",
"OOT_MAGIC_UPGRADE2",
"OOT_DEFENSE_UPGRADE",
"OOT_PROGRESSIVE_SHIELD_DEKU",
"OOT_PROGRESSIVE_SHIELD_HYLIAN",
"OOT_SMALL_KEY_FOREST",
"OOT_SMALL_KEY_FIRE",
"OOT_SMALL_KEY_WATER",
"OOT_SMALL_KEY_SPIRIT",
"OOT_SMALL_KEY_SHADOW",
"OOT_SMALL_KEY_GANON",
"OOT_SMALL_KEY_BOTW",
"OOT_SMALL_KEY_GF",
"OOT_SMALL_KEY_GTG",
"OOT_BOSS_KEY_FOREST",
"OOT_BOSS_KEY_FIRE",
"OOT_BOSS_KEY_WATER",
"OOT_BOSS_KEY_SPIRIT",
"OOT_BOSS_KEY_SHADOW",
"OOT_BOSS_KEY_GANON",
"OOT_MAP_DT",
"OOT_MAP_DC",
"OOT_MAP_JJ",
"OOT_MAP_FOREST",
"OOT_MAP_FIRE",
"OOT_MAP_WATER",
"OOT_MAP_SPIRIT",
"OOT_MAP_SHADOW",
"OOT_MAP_BOTW",
"OOT_MAP_IC",
"OOT_COMPASS_DT",
"OOT_COMPASS_DC",
"OOT_COMPASS_JJ",
"OOT_COMPASS_FOREST",
"OOT_COMPASS_FIRE",
"OOT_COMPASS_WATER",
"OOT_COMPASS_SPIRIT",
"OOT_COMPASS_SHADOW",
"OOT_COMPASS_BOTW",
"OOT_COMPASS_IC",
"OOT_WALLET",
"OOT_WALLET4",
"OOT_TRIFORCE",
"OOT_WALLET5",
"OOT_RUPEE_SILVER_DC",
"OOT_RUPEE_SILVER_BOTW",
"OOT_RUPEE_SILVER_SPIRIT_CHILD",
"OOT_RUPEE_SILVER_SPIRIT_SUN",
"OOT_RUPEE_SILVER_SPIRIT_BOULDERS",
"OOT_RUPEE_SILVER_SPIRIT_LOBBY",
"OOT_RUPEE_SILVER_SPIRIT_ADULT",
"OOT_RUPEE_SILVER_SHADOW_SCYTHE",
"OOT_RUPEE_SILVER_SHADOW_PIT",
"OOT_RUPEE_SILVER_SHADOW_SPIKES",
"OOT_RUPEE_SILVER_SHADOW_BLADES",
"OOT_RUPEE_SILVER_IC_SCYTHE",
"OOT_RUPEE_SILVER_IC_BLOCK",
"OOT_RUPEE_SILVER_GTG_SLOPES",
"OOT_RUPEE_SILVER_GTG_LAVA",
"OOT_RUPEE_SILVER_GTG_WATER",
"OOT_RUPEE_SILVER_GANON_SPIRIT",
"OOT_RUPEE_SILVER_GANON_LIGHT",
"OOT_RUPEE_SILVER_GANON_FIRE",
"OOT_RUPEE_SILVER_GANON_FOREST",
"OOT_RUPEE_SILVER_GANON_SHADOW",
"OOT_RUPEE_SILVER_GANON_WATER",
"OOT_SOUL_ENEMY_STALFOS",
"OOT_SOUL_ENEMY_OCTOROK",
"OOT_SOUL_ENEMY_WALLMASTER",
"OOT_SOUL_ENEMY_DODONGO",
"OOT_SOUL_ENEMY_KEESE",
"OOT_SOUL_ENEMY_TEKTITE",
"OOT_SOUL_ENEMY_LEEVER",
"OOT_SOUL_ENEMY_PEAHAT",
"OOT_SOUL_ENEMY_LIZALFOS_DINOLFOS",
"OOT_SOUL_ENEMY_GOHMA_LARVA",
"OOT_SOUL_ENEMY_SHABOM",
"OOT_SOUL_ENEMY_BABY_DODONGO",
"OOT_SOUL_ENEMY_DARK_LINK",
"OOT_SOUL_ENEMY_BIRI_BARI",
"OOT_SOUL_ENEMY_TAILPASARN",
"OOT_SOUL_ENEMY_PARASITE",
"OOT_SOUL_ENEMY_SKULLTULA",
"OOT_SOUL_ENEMY_TORCH_SLUG",
"OOT_SOUL_ENEMY_MOBLIN",
"OOT_SOUL_ENEMY_ARMOS",
"OOT_SOUL_ENEMY_DEKU_BABA",
"OOT_SOUL_ENEMY_DEKU_SCRUB",
"OOT_SOUL_ENEMY_BUBBLE",
"OOT_SOUL_ENEMY_BEAMOS",
"OOT_SOUL_ENEMY_FLOORMASTER",
"OOT_SOUL_ENEMY_REDEAD_GIBDO",
"OOT_SOUL_ENEMY_SKULLWALLTULA",
"OOT_SOUL_ENEMY_FLARE_DANCER",
"OOT_SOUL_ENEMY_DEAD_HAND",
"OOT_SOUL_ENEMY_SHELL_BLADE",
"OOT_SOUL_ENEMY_LIKE_LIKE",
"OOT_SOUL_ENEMY_SPIKE",
"OOT_SOUL_ENEMY_ANUBIS",
"OOT_SOUL_ENEMY_IRON_KNUCKLE",
"OOT_SOUL_ENEMY_SKULL_KID",
"OOT_SOUL_ENEMY_FLYING_POT",
"OOT_SOUL_ENEMY_FREEZARD",
"OOT_SOUL_ENEMY_STINGER",
"OOT_SOUL_ENEMY_WOLFOS",
"OOT_SOUL_ENEMY_STALCHILD",
"OOT_SOUL_ENEMY_GUAY",
"OOT_SOUL_ENEMY_THIEVES",
"OOT_SOUL_ENEMY_POE",
"OOT_SOUL_BOSS_QUEEN_GOHMA",
"OOT_SOUL_BOSS_KING_DODONGO",
"OOT_SOUL_BOSS_BARINADE",
"OOT_SOUL_BOSS_PHANTOM_GANON",
"OOT_SOUL_BOSS_VOLVAGIA",
"OOT_SOUL_BOSS_MORPHA",
"OOT_SOUL_BOSS_BONGO_BONGO",
"OOT_SOUL_BOSS_TWINROVA",
"OOT_SOUL_NPC_SARIA",
"OOT_SOUL_NPC_DARUNIA",
"OOT_SOUL_NPC_RUTO",
"OOT_SOUL_NPC_KING_ZORA",
"OOT_SOUL_NPC_ZELDA",
"OOT_SOUL_NPC_SHEIK",
"OOT_SOUL_NPC_ANJU",
"OOT_SOUL_NPC_CARPENTERS",
"OOT_SOUL_NPC_GURU_GURU",
"OOT_SOUL_NPC_MIDO",
"OOT_SOUL_NPC_KOKIRI",
"OOT_SOUL_NPC_KOKIRI_SHOPKEEPER",
"OOT_SOUL_NPC_HYLIAN_GUARD",
"OOT_SOUL_NPC_CITIZEN",
"OOT_SOUL_NPC_MALON",
"OOT_SOUL_NPC_TALON",
"OOT_SOUL_NPC_FISHING_POND_OWNER",
"OOT_SOUL_NPC_GORON",
"OOT_SOUL_NPC_MEDIGORON",
"OOT_SOUL_NPC_BIGGORON",
"OOT_SOUL_NPC_GORON_CHILD",
"OOT_SOUL_NPC_GORON_SHOPKEEPER",
"OOT_SOUL_NPC_ZORA",
"OOT_SOUL_NPC_ZORA_SHOPKEEPER",
"OOT_SOUL_NPC_BAZAAR_SHOPKEEPER",
"OOT_SOUL_NPC_BOMBCHU_SHOPKEEPER",
"OOT_SOUL_NPC_POTION_SHOPKEEPER",
"OOT_SOUL_NPC_BOMBCHU_BOWLING_LADY",
"OOT_SOUL_NPC_CHEST_GAME_OWNER",
"OOT_SOUL_NPC_SHOOTING_GALLERY_OWNER",
"OOT_SOUL_NPC_DAMPE",
"OOT_SOUL_NPC_BOMBERS",
"OOT_SOUL_NPC_POE_COLLECTOR",
"OOT_SOUL_NPC_COMPOSER_BROS",
"OOT_SOUL_NPC_HONEY_DARLING",
"OOT_SOUL_NPC_ASTRONOMER",
"OOT_SOUL_NPC_ROOFTOP_MAN",
"OOT_SOUL_NPC_BEAN_SALESMAN",
"OOT_SOUL_NPC_SCIENTIST",
"OOT_SOUL_NPC_GORMAN",
"OOT_SOUL_NPC_GROG",
"OOT_SOUL_NPC_DOG_LADY",
"OOT_SOUL_NPC_CARPET_MAN",
"OOT_SOUL_NPC_OLD_HAG",
"OOT_SOUL_NPC_BANKER",
"OOT_SOUL_NPC_THIEVES",
"OOT_SOUL_ANIMAL_CUCCO",
"OOT_SOUL_ANIMAL_COW",
"OOT_SOUL_ANIMAL_DOG",
"OOT_SOUL_ANIMAL_BUTTERFLY",
"OOT_SOUL_MISC_GS",
"OOT_SOUL_MISC_BUSINESS_SCRUB",
"OOT_KEY_RING",
"OOT_KEY_RING_FOREST",
"OOT_KEY_RING_FIRE",
"OOT_KEY_RING_WATER",
"OOT_KEY_RING_SPIRIT",
"OOT_KEY_RING_SHADOW",
"OOT_KEY_RING_GANON",
"OOT_KEY_RING_BOTW",
"OOT_KEY_RING_GF",
"OOT_KEY_RING_GTG",
"OOT_SKELETON_KEY",
"OOT_COIN_RED",
"OOT_COIN_GREEN",
"OOT_COIN_BLUE",
"OOT_COIN_YELLOW",
"OOT_TRIFORCE_POWER",
"OOT_TRIFORCE_COURAGE",
"OOT_TRIFORCE_WISDOM",
"OOT_POUCH_SILVER_DC",
"OOT_POUCH_SILVER_BOTW",
"OOT_POUCH_SILVER_SPIRIT_CHILD",
"OOT_POUCH_SILVER_SPIRIT_SUN",
"OOT_POUCH_SILVER_SPIRIT_BOULDERS",
"OOT_POUCH_SILVER_SPIRIT_LOBBY",
"OOT_POUCH_SILVER_SPIRIT_ADULT",
"OOT_POUCH_SILVER_SHADOW_SCYTHE",
"OOT_POUCH_SILVER_SHADOW_PIT",
"OOT_POUCH_SILVER_SHADOW_SPIKES",
"OOT_POUCH_SILVER_SHADOW_BLADES",
"OOT_POUCH_SILVER_IC_SCYTHE",
"OOT_POUCH_SILVER_IC_BLOCK",
"OOT_POUCH_SILVER_GTG_SLOPES",
"OOT_POUCH_SILVER_GTG_LAVA",
"OOT_POUCH_SILVER_GTG_WATER",
"OOT_POUCH_SILVER_GANON_SPIRIT",
"OOT_POUCH_SILVER_GANON_LIGHT",
"OOT_POUCH_SILVER_GANON_FIRE",
"OOT_POUCH_SILVER_GANON_FOREST",
"OOT_POUCH_SILVER_GANON_SHADOW",
"OOT_POUCH_SILVER_GANON_WATER",
"OOT_RUPEE_MAGICAL",
"OOT_BUTTON_A",
"OOT_BUTTON_C_RIGHT",
"OOT_BUTTON_C_LEFT",
"OOT_BUTTON_C_UP",
"OOT_BUTTON_C_DOWN",
"OOT_SMALL_KEY_TCG",
"OOT_KEY_RING_TCG",
"OOT_BOTTLE_POTION_RED",
"OOT_BOTTLE_POTION_GREEN",
"OOT_BOTTLE_POTION_BLUE",
"OOT_BOTTLE_FAIRY",
"OOT_BOTTLE_POE",
"OOT_BOTTLE_BIG_POE",
"OOT_BOTTLE_BLUE_FIRE",
"OOT_FISHING_POND_CHILD_FISH_2LBS",
"OOT_FISHING_POND_CHILD_FISH_3LBS",
"OOT_FISHING_POND_CHILD_FISH_4LBS",
"OOT_FISHING_POND_CHILD_FISH_5LBS",
"OOT_FISHING_POND_CHILD_FISH_6LBS",
"OOT_FISHING_POND_CHILD_FISH_7LBS",
"OOT_FISHING_POND_CHILD_FISH_8LBS",
"OOT_FISHING_POND_CHILD_FISH_9LBS",
"OOT_FISHING_POND_CHILD_FISH_10LBS",
"OOT_FISHING_POND_CHILD_FISH_11LBS",
"OOT_FISHING_POND_CHILD_FISH_12LBS",
"OOT_FISHING_POND_CHILD_FISH_13LBS",
"OOT_FISHING_POND_CHILD_FISH_14LBS",
"OOT_FISHING_POND_ADULT_FISH_4LBS",
"OOT_FISHING_POND_ADULT_FISH_5LBS",
"OOT_FISHING_POND_ADULT_FISH_6LBS",
"OOT_FISHING_POND_ADULT_FISH_7LBS",
"OOT_FISHING_POND_ADULT_FISH_8LBS",
"OOT_FISHING_POND_ADULT_FISH_9LBS",
"OOT_FISHING_POND_ADULT_FISH_10LBS",
"OOT_FISHING_POND_ADULT_FISH_11LBS",
"OOT_FISHING_POND_ADULT_FISH_12LBS",
"OOT_FISHING_POND_ADULT_FISH_13LBS",
"OOT_FISHING_POND_ADULT_FISH_14LBS",
"OOT_FISHING_POND_ADULT_FISH_15LBS",
"OOT_FISHING_POND_ADULT_FISH_16LBS",
"OOT_FISHING_POND_ADULT_FISH_17LBS",
"OOT_FISHING_POND_ADULT_FISH_18LBS",
"OOT_FISHING_POND_ADULT_FISH_19LBS",
"OOT_FISHING_POND_ADULT_FISH_20LBS",
"OOT_FISHING_POND_ADULT_FISH_21LBS",
"OOT_FISHING_POND_ADULT_FISH_22LBS",
"OOT_FISHING_POND_ADULT_FISH_23LBS",
"OOT_FISHING_POND_ADULT_FISH_24LBS",
"OOT_FISHING_POND_ADULT_FISH_25LBS",
"OOT_FISHING_POND_CHILD_LOACH_14LBS",
"OOT_FISHING_POND_CHILD_LOACH_15LBS",
"OOT_FISHING_POND_CHILD_LOACH_16LBS",
"OOT_FISHING_POND_CHILD_LOACH_17LBS",
"OOT_FISHING_POND_CHILD_LOACH_18LBS",
"OOT_FISHING_POND_CHILD_LOACH_19LBS",
"OOT_FISHING_POND_ADULT_LOACH_29LBS",
"OOT_FISHING_POND_ADULT_LOACH_30LBS",
"OOT_FISHING_POND_ADULT_LOACH_31LBS",
"OOT_FISHING_POND_ADULT_LOACH_32LBS",
"OOT_FISHING_POND_ADULT_LOACH_33LBS",
"OOT_FISHING_POND_ADULT_LOACH_34LBS",
"OOT_FISHING_POND_ADULT_LOACH_35LBS",
"OOT_FISHING_POND_ADULT_LOACH_36LBS",
"OOT_BOMBCHU_BAG",
"OOT_BOMBCHU_BAG2",
"OOT_BOMBCHU_BAG3",
"OOT_BOMBCHU_BAG_FIRST_5",
"OOT_BOMBCHU_BAG_FIRST_10",
"OOT_BOMBCHU_BAG_FIRST_20",
"OOT_SWORD_RAZOR",
"OOT_SWORD_GILDED",
"OOT_SPIN_UPGRADE",
"OOT_TRIFORCE_FULL",
"OOT_FAIRY_BIG",
"OOT_MASK_BLAST",
"OOT_MASK_STONE",
"OOT_SCALE_BRONZE",
"OOT_WEIRD_MUSHROOM",
"OOT_CHATEAU",
"OOT_GOLD_DUST",
"OOT_SEAHORSE2",
"OOT_BOTTLED_GOLD_DUST",
"OOT_BOTTLE_CHATEAU",
"OOT_TRAP_ICE",
"OOT_TRAP_FIRE",
"OOT_TRAP_SHOCK",
"OOT_TRAP_DRAIN",
"OOT_TRAP_ANTI_MAGIC",
"OOT_TRAP_KNOCKBACK",
"OOT_TRAP_RUPOOR",
"OOT_PLATINUM_TOKEN",
"MM_RUPEE_GREEN",
"MM_RUPEE_BLUE",
"MM_RUPEE_RED_10",
"MM_RUPEE_RED",
"MM_RUPEE_PURPLE",
"MM_RUPEE_SILVER",
"MM_RUPEE_GOLD",
"MM_WALLET2",
"MM_WALLET3",
"MM_RECOVERY_HEART",
"MM_HEART_PIECE",
"MM_HEART_CONTAINER",
"MM_MAGIC_JAR_SMALL",
"MM_MAGIC_JAR_LARGE",
"MM_STRAY_FAIRY",
"MM_STRAY_FAIRY_WF",
"MM_STRAY_FAIRY_SH",
"MM_STRAY_FAIRY_GB",
"MM_STRAY_FAIRY_ST",
"MM_STRAY_FAIRY_TOWN",
"MM_TRANSCENDENT_FAIRY",
"MM_BOMB",
"MM_BOMBS_5",
"MM_BOMBS_10",
"MM_BOMBS_20",
"MM_BOMBS_30",
"MM_STICK",
"MM_BOMBCHU_10",
"MM_BOMB_BAG",
"MM_BOMB_BAG2",
"MM_BOMB_BAG3",
"MM_ARROWS_10",
"MM_ARROWS_30",
"MM_ARROWS_40",
"MM_UNK_21",
"MM_BOW",
"MM_QUIVER2",
"MM_QUIVER3",
"MM_ARROW_FIRE",
"MM_ARROW_ICE",
"MM_ARROW_LIGHT",
"MM_NUT",
"MM_NUTS_5",
"MM_NUTS_10",
"MM_UNK_2B",
"MM_UNK_2C",
"MM_UNK_2D",
"MM_BOMBCHU_20",
"MM_UNK_2F",
"MM_UNK_30",
"MM_UNK_31",
"MM_SHIELD_DEKU",
"MM_SHIELD_HERO",
"MM_SHIELD_MIRROR",
"MM_PROGRESSIVE_SHIELD_DEKU",
"MM_PROGRESSIVE_SHIELD_HERO",
"MM_POWDER_KEG",
"MM_MAGIC_BEAN",
"MM_BOMBCHU",
"MM_SWORD_KOKIRI",
"MM_SWORD_RAZOR",
"MM_SWORD_GILDED",
"MM_BOMBCHU_5",
"MM_GREAT_FAIRY_SWORD",
"MM_SMALL_KEY",
"MM_BOSS_KEY",
"MM_MAP",
"MM_COMPASS",
"MM_HOOKSHOT",
"MM_LENS",
"MM_PICTOGRAPH_BOX",
"MM_OCARINA_OF_TIME",
"MM_BOMBER_NOTEBOOK",
"MM_BOTTLE_POTION_RED",
"MM_BOTTLE_EMPTY",
"MM_POTION_RED",
"MM_POTION_GREEN",
"MM_POTION_BLUE",
"MM_FAIRY",
"MM_BOTTLE_MILK",
"MM_FISH",
"MM_BUGS",
"MM_BIG_POE",
"MM_BOTTLED_GOLD_DUST",
"MM_WEIRD_MUSHROOM",
"MM_BOTTLE_CHATEAU",
"MM_MASK_DEKU",
"MM_MASK_GORON",
"MM_MASK_ZORA",
"MM_MASK_FIERCE_DEITY",
"MM_MASK_CAPTAIN",
"MM_MASK_GIANT",
"MM_MASK_ALL_NIGHT",
"MM_MASK_BUNNY",
"MM_MASK_KEATON",
"MM_MASK_GARO",
"MM_MASK_ROMANI",
"MM_MASK_TROUPE_LEADER",
"MM_MASK_POSTMAN",
"MM_MASK_COUPLE",
"MM_MASK_GREAT_FAIRY",
"MM_MASK_GIBDO",
"MM_MASK_DON_GERO",
"MM_MASK_KAMARO",
"MM_MASK_TRUTH",
"MM_MASK_STONE",
"MM_MASK_BREMEN",
"MM_MASK_BLAST",
"MM_MASK_SCENTS",
"MM_MASK_KAFEI",
"MM_CHATEAU",
"MM_MILK",
"MM_GOLD_DUST",
"MM_SEAHORSE2",
"MM_MOON_TEAR",
"MM_DEED_LAND",
"MM_DEED_SWAMP",
"MM_DEED_MOUNTAIN",
"MM_DEED_OCEAN",
"MM_ROOM_KEY",
"MM_LETTER_TO_MAMA",
"MM_LETTER_TO_KAFEI",
"MM_PENDANT_OF_MEMORIES",
"MM_WORLD_MAP_CLOCK_TOWN",
"MM_WORLD_MAP_WOODFALL",
"MM_WORLD_MAP_SNOWHEAD",
"MM_WORLD_MAP_ROMANI_RANCH",
"MM_WORLD_MAP_GREAT_BAY",
"MM_WORLD_MAP_STONE_TOWER",
"MM_MAGIC_UPGRADE",
"MM_MAGIC_UPGRADE2",
"MM_DEFENSE_UPGRADE",
"MM_SPIN_UPGRADE",
"MM_SONG_AWAKENING",
"MM_SONG_GORON",
"MM_SONG_ZORA",
"MM_SONG_EMPTINESS",
"MM_SONG_ORDER",
"MM_SONG_TIME",
"MM_SONG_HEALING",
"MM_SONG_EPONA",
"MM_SONG_SOARING",
"MM_SONG_OF_STORMS",
"MM_SONG_SUN",
"MM_SONG_GORON_HALF",
"MM_SONG_NOTE_AWAKENING",
"MM_SONG_NOTE_GORON",
"MM_SONG_NOTE_ZORA",
"MM_SONG_NOTE_EMPTINESS",
"MM_SONG_NOTE_ORDER",
"MM_SONG_NOTE_TIME",
"MM_SONG_NOTE_HEALING",
"MM_SONG_NOTE_EPONA",
"MM_SONG_NOTE_SOARING",
"MM_SONG_NOTE_STORMS",
"MM_SONG_NOTE_SUN",
"MM_REMAINS_ODOLWA",
"MM_REMAINS_GOHT",
"MM_REMAINS_GYORG",
"MM_REMAINS_TWINMOLD",
"MM_GS_TOKEN_SWAMP",
"MM_GS_TOKEN_OCEAN",
"MM_PLATINUM_TOKEN",
"MM_SMALL_KEY_WF",
"MM_SMALL_KEY_SH",
"MM_SMALL_KEY_GB",
"MM_SMALL_KEY_ST",
"MM_BOSS_KEY_WF",
"MM_BOSS_KEY_SH",
"MM_BOSS_KEY_GB",
"MM_BOSS_KEY_ST",
"MM_MAP_WF",
"MM_MAP_SH",
"MM_MAP_GB",
"MM_MAP_ST",
"MM_COMPASS_WF",
"MM_COMPASS_SH",
"MM_COMPASS_GB",
"MM_COMPASS_ST",
"MM_OCARINA_FAIRY",
"MM_HOOKSHOT_SHORT",
"MM_WALLET",
"MM_WALLET4",
"MM_OWL_GREAT_BAY",
"MM_OWL_ZORA_CAPE",
"MM_OWL_SNOWHEAD",
"MM_OWL_MOUNTAIN_VILLAGE",
"MM_OWL_CLOCK_TOWN",
"MM_OWL_MILK_ROAD",
"MM_OWL_WOODFALL",
"MM_OWL_SOUTHERN_SWAMP",
"MM_OWL_IKANA_CANYON",
"MM_OWL_STONE_TOWER",
"MM_OWL_HIDDEN",
"MM_WALLET5",
"MM_SOUL_ENEMY_OCTOROK",
"MM_SOUL_ENEMY_WALLMASTER",
"MM_SOUL_ENEMY_DODONGO",
"MM_SOUL_ENEMY_KEESE",
"MM_SOUL_ENEMY_TEKTITE",
"MM_SOUL_ENEMY_PEAHAT",
"MM_SOUL_ENEMY_LIZALFOS_DINOLFOS",
"MM_SOUL_ENEMY_SKULLTULA",
"MM_SOUL_ENEMY_ARMOS",
"MM_SOUL_ENEMY_DEKU_BABA",
"MM_SOUL_ENEMY_DEKU_SCRUB",
"MM_SOUL_ENEMY_BUBBLE",
"MM_SOUL_ENEMY_BEAMOS",
"MM_SOUL_ENEMY_REDEAD_GIBDO",
"MM_SOUL_ENEMY_SKULLWALLTULA",
"MM_SOUL_ENEMY_SHELL_BLADE",
"MM_SOUL_ENEMY_LIKE_LIKE",
"MM_SOUL_ENEMY_IRON_KNUCKLE",
"MM_SOUL_ENEMY_FREEZARD",
"MM_SOUL_ENEMY_WOLFOS",
"MM_SOUL_ENEMY_GUAY",
"MM_SOUL_ENEMY_FLYING_POT",
"MM_SOUL_ENEMY_FLOORMASTER",
"MM_SOUL_ENEMY_CHUCHU",
"MM_SOUL_ENEMY_DEEP_PYTHON",
"MM_SOUL_ENEMY_STALCHILD",
"MM_SOUL_ENEMY_LEEVER",
"MM_SOUL_ENEMY_SKULLFISH",
"MM_SOUL_ENEMY_DEXIHAND",
"MM_SOUL_ENEMY_DRAGONFLY",
"MM_SOUL_ENEMY_EENO",
"MM_SOUL_ENEMY_EYEGORE",
"MM_SOUL_ENEMY_HIPLOOP",
"MM_SOUL_ENEMY_REAL_BOMBCHU",
"MM_SOUL_ENEMY_TAKKURI",
"MM_SOUL_ENEMY_BOE",
"MM_SOUL_ENEMY_NEJIRON",
"MM_SOUL_ENEMY_BIO_BABA",
"MM_SOUL_ENEMY_GARO",
"MM_SOUL_ENEMY_WIZZROBE",
"MM_SOUL_ENEMY_GOMESS",
"MM_SOUL_ENEMY_GEKKO",
"MM_SOUL_ENEMY_BAD_BAT",
"MM_SOUL_ENEMY_SNAPPER",
"MM_SOUL_ENEMY_WART",
"MM_SOUL_ENEMY_CAPTAIN_KEETA",
"MM_SOUL_ENEMY_THIEVES",
"MM_SOUL_ENEMY_POE",
"MM_SOUL_BOSS_ODOLWA",
"MM_SOUL_BOSS_GOHT",
"MM_SOUL_BOSS_GYORG",
"MM_SOUL_BOSS_TWINMOLD",
"MM_SOUL_BOSS_IGOS",
"MM_SOUL_NPC_AROMA",
"MM_SOUL_NPC_SHOOTING_GALLERY_OWNER",
"MM_SOUL_NPC_BAZAAR_SHOPKEEPER",
"MM_SOUL_NPC_GORON",
"MM_SOUL_NPC_GORON_CHILD",
"MM_SOUL_NPC_BOMBCHU_SHOPKEEPER",
"MM_SOUL_NPC_BOMBERS",
"MM_SOUL_NPC_BUTLER_DEKU",
"MM_SOUL_NPC_CITIZEN",
"MM_SOUL_NPC_COMPOSER_BROS",
"MM_SOUL_NPC_DAMPE",
"MM_SOUL_NPC_DEKU_KING",
"MM_SOUL_NPC_DEKU_PRINCESS",
"MM_SOUL_NPC_PLAYGROUND_SCRUBS",
"MM_SOUL_NPC_GORON_ELDER",
"MM_SOUL_NPC_CHEST_GAME_OWNER",
"MM_SOUL_NPC_GORON_SHOPKEEPER",
"MM_SOUL_NPC_HONEY_DARLING",
"MM_SOUL_NPC_TOTO",
"MM_SOUL_NPC_RUTO",
"MM_SOUL_NPC_ZORA_MUSICIANS",
"MM_SOUL_NPC_KAFEI",
"MM_SOUL_NPC_KEATON",
"MM_SOUL_NPC_KOUME_KOTAKE",
"MM_SOUL_NPC_MAYOR_DOTOUR",
"MM_SOUL_NPC_MEDIGORON",
"MM_SOUL_NPC_BIGGORON",
"MM_SOUL_NPC_TALON",
"MM_SOUL_NPC_ASTRONOMER",
"MM_SOUL_NPC_POE_COLLECTOR",
"MM_SOUL_NPC_BLACKSMITHS",
"MM_SOUL_NPC_BOMBCHU_BOWLING_LADY",
"MM_SOUL_NPC_TINGLE",
"MM_SOUL_NPC_TOILET_HAND",
"MM_SOUL_NPC_FISHING_POND_OWNER",
"MM_SOUL_NPC_ROOFTOP_MAN",
"MM_SOUL_NPC_ZORA",
"MM_SOUL_NPC_ZORA_SHOPKEEPER",
"MM_SOUL_NPC_MALON",
"MM_SOUL_NPC_BEAN_SALESMAN",
"MM_SOUL_NPC_CARPENTERS",
"MM_SOUL_NPC_ANJU",
"MM_SOUL_NPC_GURU_GURU",
"MM_SOUL_NPC_SCIENTIST",
"MM_SOUL_NPC_GORMAN",
"MM_SOUL_NPC_GROG",
"MM_SOUL_NPC_DOG_LADY",
"MM_SOUL_NPC_CARPET_MAN",
"MM_SOUL_NPC_OLD_HAG",
"MM_SOUL_NPC_TOURIST_CENTER",
"MM_SOUL_NPC_BANKER",
"MM_SOUL_NPC_MOON_CHILDREN",
"MM_SOUL_NPC_THIEVES",
"MM_SOUL_ANIMAL_CUCCO",
"MM_SOUL_ANIMAL_COW",
"MM_SOUL_ANIMAL_DOG",
"MM_SOUL_ANIMAL_BUTTERFLY",
"MM_SOUL_MISC_GS",
"MM_SOUL_MISC_BUSINESS_SCRUB",
"MM_KEY_RING",
"MM_KEY_RING_WF",
"MM_KEY_RING_SH",
"MM_KEY_RING_GB",
"MM_KEY_RING_ST",
"MM_SKELETON_KEY",
"MM_BUTTON_A",
"MM_BUTTON_C_RIGHT",
"MM_BUTTON_C_LEFT",
"MM_BUTTON_C_UP",
"MM_BUTTON_C_DOWN",
"MM_BOTTLE_POTION_GREEN",
"MM_BOTTLE_POTION_BLUE",
"MM_BOTTLE_FAIRY",
"MM_POE",
"MM_BOTTLE_POE",
"MM_BOTTLE_BIG_POE",
"MM_BOMBCHU_BAG",
"MM_BOMBCHU_BAG2",
"MM_BOMBCHU_BAG3",
"MM_BOMBCHU_BAG_FIRST_1",
"MM_BOMBCHU_BAG_FIRST_5",
"MM_BOMBCHU_BAG_FIRST_10",
"MM_BOMBCHU_BAG_FIRST_20",
"MM_FAIRY_BIG",
"MM_SPELL_FIRE",
"MM_SPELL_WIND",
"MM_SPELL_LOVE",
"MM_BOOTS_IRON",
"MM_BOOTS_HOVER",
"MM_TUNIC_GORON",
"MM_TUNIC_ZORA",
"MM_SCALE_BRONZE",
"MM_SCALE_SILVER",
"MM_SCALE_GOLDEN",
"MM_GORON_BRACELET",
"MM_SILVER_GAUNTLETS",
"MM_GOLDEN_GAUNTLETS",
"MM_CLOCK1",
"MM_CLOCK2",
"MM_CLOCK3",
"MM_CLOCK4",
"MM_CLOCK5",
"MM_CLOCK6",
"MM_HAMMER",
"MM_STICK_UPGRADE2",
"MM_STICK_UPGRADE3",
"MM_NUT_UPGRADE2",
"MM_NUT_UPGRADE3",
"MM_STONE_OF_AGONY",
"MM_BOTTLE_RUTO_LETTER",
"MM_BOTTLE_BLUE_FIRE",
"MM_BLUE_FIRE",
"MM_MASK_MAJORA",
"MM_SOLD_OUT",
"NOTHING",
"MM_SCOOP"
]

unkItems = {
"OOT_LONGSHOT" : "Longshot (OoT)",
"OOT_OCARINA_TIME" : "Ocarina of Time (OoT)",
"OOT_QUIVER2" : "Big Quiver (OoT)",
"OOT_QUIVER3" : "Biggest Quiver (OoT)",
"OOT_BOMB_BAG2" : "Big Bomb Bag (OoT)",
"OOT_BOMB_BAG3" : "Biggest Bomb Bag (OoT)",
"OOT_SILVER_GAUNTLETS" : "Silver Gauntlets (OoT)",
"OOT_GOLDEN_GAUNTLETS" : "Golden Gauntlets (OoT)",
"OOT_SCALE_SILVER" : "Silver Scale (OoT)",
"OOT_SCALE_GOLDEN" : "Golden Scale (OoT)",
"OOT_OCARINA_FAIRY" : "Fairy Ocarina (OoT)",
"OOT_BOSS_KEY": "Boss Key (OoT)",
"OOT_COMPASS": "Compass (OoT)",
"OOT_MAP": "Map (OoT)",
"OOT_SMALL_KEY": "Small Key (OoT)",
"OOT_WALLET2": "Adult's Wallet (OoT)",
"OOT_WALLET3": "Giant's Wallet (OoT)",
"OOT_HEART_CONTAINER2": "Heart Container (OoT)",
"OOT_GORON_BRACELET": "Goron's Bracelet (OoT)",
"OOT_BULLET_BAG": "Bullet Bag",
"OOT_BULLET_BAG2": "Large Bullet Bag",
"OOT_TC_SMALL_KEY": "Small Key (OoT)",
"OOT_TC_RUPEE_GREEN": "Green Rupee (OoT)",
"OOT_TC_RUPEE_BLUE": "Blue Rupee (OoT)",
"OOT_TC_RUPEE_RED": "Red Rupee (OoT)",
"OOT_TC_RUPEE_PURPLE": "Purple Rupee (OoT)",
"OOT_TC_HEART_PIECE": "Piece of Heart (OoT)",
"OOT_STICK_UPGRADE2": "Second Deku Stick Upgrade (OoT)",
"OOT_NUT_UPGRADE2": "Second Deku Nut Upgrade (OoT)",
"OOT_BULLET_BAG3": "Largest Bullet Bag",
"OOT_UNUSED_ICE_TRAP": "IceTrap (OoT)",
"OOT_UNK_7D" : "Unknown",
"OOT_UNK_7E" : "Unknown",
"OOT_UNK_7F" : "Unknown",
"OOT_MAGIC_UPGRADE2": "Larger Magic Upgrade (OoT)",
"OOT_PROGRESSIVE_SHIELD_DEKU": "Progressive Deku Shield (OoT)",
"OOT_PROGRESSIVE_SHIELD_HYLIAN": "Progressive Hylian Shield",
"OOT_WALLET": "Child's Wallet (OoT)",
"OOT_WALLET4": "Colossal Wallet (OoT)",
"OOT_WALLET5": "Bottomless Wallet (OoT)",
"OOT_TRIFORCE": "Triforce Piece",
"OOT_KEY_RING": "Key Ring (OoT)",
"OOT_TRIFORCE_POWER": "Triforce of Power",
"OOT_TRIFORCE_COURAGE": "Triforce of Courage",
"OOT_TRIFORCE_WISDOM": "Triforce of Wisdom",
"OOT_BOMBCHU_BAG2": "Big Bombchu Bag (OoT)",
"OOT_BOMBCHU_BAG3": "Biggest Bombchu Bag (OoT)",
"OOT_BOMBCHU_BAG_FIRST_5": "Bombchu Bag (OoT)",
"OOT_BOMBCHU_BAG_FIRST_10": "Bombchu Bag (OoT)",
"OOT_BOMBCHU_BAG_FIRST_20": "Bombchu Bag (OoT)",
"OOT_SWORD_RAZOR": "Razor Sword (OoT)",
"OOT_SWORD_GILDED": "Gilded Sword (OoT)",
"OOT_TRIFORCE_FULL": "Triforce (OoT)",
"OOT_SCALE_BRONZE": "Bronze Scale (OoT)",
"OOT_WEIRD_MUSHROOM": "Magic Mushroom",
"OOT_GOLD_DUST": "Chateau Romani",
"OOT_SEAHORSE2": "Seahorse",
"MM_RUPEE_RED_10": "Red Rupee (MM)",
"MM_WALLET2": "Adult Wallet (MM)",
"MM_WALLET3": "Giant Wallet (MM)",
"MM_STRAY_FAIRY": "Stray Fairy (MM)",
"MM_BOMB_BAG2": "Big Bomb Bag (MM)",
"MM_BOMB_BAG3": "Biggest Bomb Bag (MM)",
"MM_UNK_21" : "40 Arrows (MM)",
"MM_QUIVER2": "Big Quiver (MM)",
"MM_QUIVER3": "Biggest Quiver (MM)",
"MM_UNK_2B" : "30 Deku Nuts (MM)",
"MM_UNK_2C" : "30 Deku Nuts (MM)",
"MM_UNK_2D" : "40 Deku Nuts (MM)",
"MM_UNK_2F" : "20 Deku Sticks (MM)",
"MM_UNK_30" : "20 Deku Sticks (MM)",
"MM_UNK_31" : "30 Deku Sticks (MM)",
"MM_PROGRESSIVE_SHIELD_DEKU": "Progressive Deku Shield (MM)",
"MM_PROGRESSIVE_SHIELD_HERO": "Progressive Hero's Shield",
"MM_SWORD_KOKIRI": "Kokiri Sword (MM)",
"MM_SWORD_RAZOR": "Razor Sword (MM)",
"MM_SWORD_GILDED": "Gilded Sword (MM)",
"MM_SMALL_KEY": "Small Key (MM)",
"MM_BOSS_KEY": "Boss Key (MM)",
"MM_MAP": "Map (MM)",
"MM_COMPASS": "Compass (MM)",
"MM_OCARINA_OF_TIME": "Ocarina of Time (MM)",
"MM_WEIRD_MUSHROOM": "Magic Mushroom",
"MM_GOLD_DUST": "Gold Dust",
"MM_SEAHORSE2": "Seahorse",
"MM_MAGIC_UPGRADE2": "Larger Magic Upgrade (MM)",
"MM_OCARINA_FAIRY": "Fairy Ocarina (MM)",
"MM_HOOKSHOT_SHORT": "Short Hookshot (MM)",
"MM_WALLET": "Child Wallet (MM)",
"MM_WALLET4": "Colossal Wallet (MM)",
"MM_OWL_HIDDEN": "Owl Statue (Hidden)",
"MM_WALLET5": "Bottomless Wallet (MM)",
"MM_KEY_RING": "Key Ring (MM)",
"MM_BOMBCHU_BAG2": "Big Bombchu Bag (MM)",
"MM_BOMBCHU_BAG3": "Biggest Bombchu Bag (MM)",
"MM_BOMBCHU_BAG_FIRST_1": "Bombchu Bag (MM)",
"MM_BOMBCHU_BAG_FIRST_5": "Bombchu Bag (MM)",
"MM_BOMBCHU_BAG_FIRST_10": "Bombchu Bag (MM)",
"MM_BOMBCHU_BAG_FIRST_20": "Bombchu Bag (MM)",
"MM_SCALE_BRONZE": "Bronze Scale (MM)",
"MM_SCALE_SILVER": "Silver Scale (MM)",
"MM_SCALE_GOLDEN": "Golden Scale (MM)",
"MM_GORON_BRACELET": "Goron's Bracelet (MM)",
"MM_SILVER_GAUNTLETS": "Silver Gauntlets (MM)",
"MM_GOLDEN_GAUNTLETS": "Golden Gauntlets (MM)",
"MM_STICK_UPGRADE3": "Second Deku Stick Upgrade (MM)",
"MM_STICK_UPGRADE2": "Deku Stick Upgrade (MM)",
"MM_STICK_UPGRADE": "Deku Stick Capacity (MM)",
"MM_NUT_UPGRADE3": "Second Deku Nut Upgrade (MM)",
"MM_NUT_UPGRADE2": "Deku Nut Upgrade (MM)",
"MM_NUT_UPGRADE": "Deku Nut Capacity (MM)",
"MM_MASK_MAJORA": "Majora's Mask",
"MM_SOLD_OUT" : "Sold Out",
"MM_SCOOP" : "MM Scoop"
}

with open(input_file, "r", newline='', encoding='utf-8') as csvfile_in, \
     open(output_file, "w", newline='', encoding='utf-8') as csvfile_out:

    reader = pd.read_csv(csvfile_in, delimiter=";", header=0)
    df = pd.DataFrame(reader)
    writer = csv.writer(csvfile_out)
    currnum = 1
    defines = "\n\n"
    for currItem in itemOrder:
        currName = df[df['ID'] == currItem]
        currRender = ""
        shared = ""
        if len(currName['Name'].values) > 0:
            tmpItem = currName
            currName = currName['Name'].values[0]
            currRender = tmpItem['Render_Type'].values[0]
            shared = str(tmpItem['Can_Be_Shared'].values[0]).lower()
        else:
            currName = unkItems[currItem]

        new_hex = f"0x{currnum:X}"  # hex en majuscules
        fin_row = "{ " + currItem + ', \"' + currName + "\", EGameIcon::" + str(currRender) + ", " + shared + " },\n"
        csvfile_out.write(fin_row)
        currnum = currnum + 1
        
        defines += "#define " + currItem + " " + new_hex + "\n"
    
    csvfile_out.write (defines)

    # --- Corps de ResolveRawItemID (stable -> dev) ----------------------------
    # Aligne l'ordre dev (itemOrder) et l'ordre stable (stableOrder) : pour chaque item present dans
    # les deux, delta = index_dev - index_stable (1-based). Une "boundary" = un point ou delta change.
    # Les items dev-only (SHARED_* / post "# Extra Adds") sont tries apres tous les items stable donc
    # ne decalent aucun ID stable et n'apparaissent jamais ici.
    dev_index = {}
    for pos, nm in enumerate(itemOrder):
        dev_index.setdefault(nm, pos + 1)   # 1-based, premiere occurrence

    missing = [nm for nm in stableOrder if nm not in dev_index]
    if missing:
        print("ATTENTION: items stable absents de itemOrder:", missing)

    boundaries = []
    prev_delta = None
    last_delta = 0
    for pos, nm in enumerate(stableOrder):
        sid = pos + 1
        if nm not in dev_index:
            continue
        delta = dev_index[nm] - sid
        if delta < last_delta:
            print(f"ATTENTION: delta non monotone a {nm} (stable {sid}): {delta} < {last_delta}")
        last_delta = delta
        if delta != prev_delta:
            boundaries.append((sid, delta, nm))
            prev_delta = delta

    resolve = "\n\n// ==== Corps de ResolveRawItemID (stable -> dev), teste haut -> bas ====\n"
    rows = [b for b in boundaries if b[1] > 0]
    for i in range(len(rows) - 1, -1, -1):
        sid, delta, nm = rows[i]
        inc = delta - (rows[i - 1][1] if i > 0 else 0)
        resolve += f"    if (RawItemID >= {sid}) return RawItemID + {delta};".ljust(52) + f"// +{inc} at {nm}\n"
    resolve += "\n    return RawItemID;\n"
    csvfile_out.write(resolve)

print(f"Fichier écrit : {output_file}")