...
void ProgressionTab::OnItemFound(int Game, ObjectInfo* Object, const ItemInfo* Item, bool IsAddOp)
{
    ...
    // OoTMM Short Hookshot (shortHookshotMm): the seed places the SAME item twice as
    // "Hookshot (MM)" (MM_HOOKSHOT) - the first pickup is the short-range hookshot, the
    // second upgrades it to full range (gi.yml: MM_HOOKSHOT_SHORT adds +1, MM_HOOKSHOT
    // +2; can_hookshot needs 2 when the setting is on). No placement is ever named
    // "Short Hookshot", so a plain match lights the full tier on the first pickup and
    // the Short tier stays dead. Walk [Short, full] on successive MM_HOOKSHOT pickups
    // instead. The setting is on exactly when MM_HOOKSHOT_SHORT is NOT disabled (off ->
    // CheckItemEnabled disables it and hides the Short tier, and a lone hookshot is
    // already full). A MM_HOOKSHOT placement only exists when the hookshot is NOT shared
    // (a shared hookshot is placed as SHARED_HOOKSHOT), so no shared guard is needed.
    if (Item->ItemID == MM_HOOKSHOT
        && this->RomSettings != nullptr
        && !this->RomSettings->DisabledItemIDs.contains(MM_HOOKSHOT_SHORT))
    {
        QList<ItemIconWidget*> stages;
        const GameProgData* shortRegistries[] = {
            &this->OoTData, &this->MMData, &this->SoulsData, &this->CollectiblesData
        };
        for (const GameProgData* reg : shortRegistries)
        {
            for (ItemIconWidget* w : reg->All)
            {
                if (w != nullptr && w->LookupKeys.contains(MM_HOOKSHOT_SHORT))
                {
                    stages.append(w);
                }
            }
        }
        stages.append(matches);   // the full Hookshot tier(s), after the Short tier
        walkStages(stages);
        return;
    }
