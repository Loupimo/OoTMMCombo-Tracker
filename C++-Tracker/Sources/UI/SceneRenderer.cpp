...
{
    // Resolve which background image to display: an active room's image wins;
    // otherwise a scene that ships a per-context map picks winter/spring (child/
    // adult) from the toggle; otherwise the single scene image.
    const char* path;
    if (Room != nullptr)
    {
        path = Room->ImagePath;
    ...
    {
        SceneMetaInfo* Info = this->CurrScene->Info;
        const char* CtxImg = Info->ContextImagePath;
        path = (Info->HasContext && Context && CtxImg && CtxImg[0] != '\0')
             ? CtxImg                           // Toggle ON  -> Spring / Adult map
             : Info->ImagePath;                 // Toggle OFF -> Winter / Child, or no per-context map
    }

    if (this->SceneImageItem == nullptr)
    {   // This is the first time the scene is rendered
        ...
        this->LoadedImagePath = path;
    }
    else if (this->LoadedImagePath != path)
    {   // The background changed (room switch, or per-context map swap). Replace the
        // pixmap in place so the item keeps its Z-order behind the object markers.

        delete this->SceneImage;
        this->SceneImage = new QPixmap(path);
        this->SceneImageItem->setPixmap(*this->SceneImage);
        this->LoadedImagePath = path;
    }
