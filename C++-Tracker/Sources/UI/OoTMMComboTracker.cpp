...
/*
*   Tell whether a spoiler "Version:" string is a stable build <= v32.3, i.e. before the xflag rework
*   that swapped the full scene/room/actor identity in the override key for a compact XflagID. Such
*   builds keep the legacy key layout and need no XflagID resolution. Dev builds and unrecognised
*   strings are treated as current (new system).
*   @param VersionString  The raw value from the spoiler's "Version:" line (e.g. "v32.3", "dev-...").
*   @return true if the build is a stable release <= v32.3, false otherwise.
*/
static bool SpoilerBuildPredatesV32_3(const QString& VersionString)
{
    if (VersionString.startsWith("dev"))
    {   // Dev builds are considered current (> v32.3): compact XflagID system.
    ...
    QRegularExpressionMatch Match = QRegularExpression("(\\d+)\\.(\\d+)").match(VersionString);
    if (!Match.hasMatch())
    {   // Unrecognised version string: assume current, use the compact XflagID system.
...
void OoTMMComboTracker::LoadGameSpoiler(QString FilePath)
{
    ...
    // builds match it as-is, stable builds report lower IDs that ResolveRawItemID shifts up.
    {
        ...
        if (versionIt.hasNext())
        {
            ...
            // Builds <= v32.3 encode the full xflag identity in the key; newer ones only carry a
            // compact XflagID that ResolveXflagItem must expand (see Combo/Xflags).
            this->ROMSettings.UsesLegacyXflags = SpoilerBuildPredatesV32_3(version);
        }

    }

    // Publish the xflag system to the item pipeline (MemoryReader / Game have no Settings handle).
    SetUsesLegacyXflags(this->ROMSettings.UsesLegacyXflags);
