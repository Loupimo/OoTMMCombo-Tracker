...
int sendqOpen(SendQueue* q, const uint8_t* uuid)
{
    ...
    _mkdir(buffer);
    strcat_s(buffer, "sendq.bin");

    /* Ensure the file exists */
    fopen_s(&q->file, buffer, "ab");
    fclose(q->file);

    /* Open the file */
    fopen_s(&q->file, buffer, "r+b");
    if (!q->file)
        return -1;
    ...
    q->size = count;
    q->capacity = count + 8;
    q->data = (SendQueueEntry*)malloc(q->capacity * sizeof(SendQueueEntry));
...
static int sendqWrite(SendQueue* sq, const LedgerFullEntry* entry)
{
    ...
    if (sq->size == sq->capacity)
    {
        ...
        if (!newData)
            return -1;
        sq->data = (SendQueueEntry*)newData;
...
static int sendqTransfer(NetBuffer* nb, const LedgerFullEntry* entry)
{
    ...
    bufDst = (char*)nb->netBufReserve(1 + 8 + 1 + entry->size);
    if (!bufDst)
        return -1;
