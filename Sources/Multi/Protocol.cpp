...
static int aresCommandWrite(Game* game, uint32_t addr, int size, uint8_t *value)
{
    ...
    error |= !sockSend(game->socketApi, "$", 1);
    sprintf_s(buf, "M%08x,%x:", addr, size);
    for (int i = 0; buf[i] != 0; ++i)
        checksum += buf[i];
    error |= !sockSend(game->socketApi, buf, (uint32_t)strlen(buf));
    for (int i = 0; i < size; i ++) {
        sprintf_s(buf, "%02x", value[i]);
        error |= !sockSend(game->socketApi, buf, (uint32_t)strlen(buf));
        checksum += buf[i];
    }
    sprintf_s(buf, "#%02x", checksum);
...
static void aresWriteInt(Game* game, uint32_t addr, uint32_t value, int size)
{
    ...
    if (!aresCommandWrite(game, addr, size, (uint8_t *)buf))
    {
        game->apiError = 1;
        closesocket(game->socketApi);
        game->socketApi = INVALID_SOCKET;
