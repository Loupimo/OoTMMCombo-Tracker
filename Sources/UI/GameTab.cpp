...
QString GameTab::GetAccentColorFor(int GameID)
{
    return GameID == OOT_GAME ? QStringLiteral("#4a9edb") : QStringLiteral("#9b5de5");
}


QString GameTab::GetAccentColor() const
{
    return GameTab::GetAccentColorFor(this->GameID);
}
...
void GameTab::SaveGameScenes(QString FilePath, Settings * FileSettings)
{
    ...
    switch (AppConfig::GetTrackerVersion())
    {
        ...
        {   // Current format: layout-resilient per-world scene objects (+ TargetWorld) and entrances.
