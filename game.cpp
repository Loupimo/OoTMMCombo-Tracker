...
void gameInit(Game* game, SOCKET sock, int apiProtocol)
{
    ...
    game->rxBuffer = (char*)malloc(sizeof(char) * 256);
    game->rxBufferSize = 0;

    game->entriesCount = 0;
    game->entriesCapacity = 256;
    game->entries = (LedgerFullEntry*)malloc(game->entriesCapacity * sizeof(LedgerFullEntry));
    ...
    game_file_log = fopen("./Logs/Game_Log.csv", "w+");
...
static void gameApiApplyLedger(Game* game)
{
    ...
    protocolWriteBuffer(game, cmdBase + 0x00, 3, (uint8_t *)fe->data);
    ...
    printf("ITEM IN - FROM: %d, TO: %d, GAME: %d, KEY: %04X, GI: %04X, FLAGS: %04X\n", playerFrom, playerTo, gameId, key, gi, flags);

    //uint8_t keyArr[5] = { (uint8_t)fe->data[2], (uint8_t)fe->data[4], (uint8_t)fe->data[5], (uint8_t)fe->data[6], (uint8_t)fe->data[7] };
    uint8_t keyArr[5] = { (uint8_t)fe->data[2], (uint8_t)fe->data[7], (uint8_t)fe->data[6], (uint8_t)fe->data[5], (uint8_t)fe->data[4] };
    ...
    uint16_t gi_in = buffer[9] << 8 | buffer[8];

    printf("OvType : %d, Scene ID: %d, Room ID: %d, Object ID: %d\n", recievedItem.OvType, recievedItem.SceneID, recievedItem.RoomID, recievedItem.ObjectID);

    ObjectInfo matchObject = FindObject(recievedItem);
    ItemInfo matchItem = FindItem(gi_in);

    if (recievedItem.GameID == OOT_GAME)
    {
        fprintf(game_file_log, "OoT %s;%02X%;%02X;%02X;%04X;%s;%02X\n", matchObject.Location, recievedItem.OvType, recievedItem.SceneID, recievedItem.RoomID, recievedItem.ObjectID, matchItem.ItemName, gi_in);
        printf("OoT World Object: %s - Item : %s\n", matchObject.Location, matchItem.ItemName);
    }
    else
    {

        fprintf(game_file_log, "MM %s;%02X%;%02X;%02X;%04X;%s;%02X\n", matchObject.Location, recievedItem.OvType, recievedItem.SceneID, recievedItem.RoomID, recievedItem.ObjectID, matchItem.ItemName, gi_in);
        printf("MM World Object: %s - Item : %s\n", matchObject.Location, matchItem.ItemName);
    }
...
static int insertMessage(Game* game, NetMsg* msg)
{
    ...
    if (msg->size > 0) protocolWriteBuffer(game, game->apiNetAddr + 0xa8 + index * 32, msg->size, (uint8_t*)msg->data);
    return 0;
}
...
static int gameProcessRxLedgerEntry(Game* game)
{
    ...
    if (game->entriesCount == game->entriesCapacity)
    {
        ...
        game->entries = (LedgerFullEntry*)realloc(game->entries, game->entriesCapacity * sizeof(LedgerFullEntry));
    }
